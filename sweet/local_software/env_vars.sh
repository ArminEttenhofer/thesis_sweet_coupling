...
#
#
# You can create a file 'mule_local_platform.conf' to set some environment
# variables specific to this source code folder.
#
# E.g., you can set
#
# 	export MULE_PLATFORM_ID=default_llvm
#
# to always load the 'default_llvm' platform for this source.
#
...
function activate_miniconda_environment()
{
	...
	cd "${MULE_SOFTWARE_ROOT}/mule/env_pythonpath" || return 1
	{ pip3 show mule > /dev/null; } || { "echo Installing 'MULE'"; make install || return 1; }
...
MLP="$MULE_SOFTWARE_ROOT/mule_local_platform.conf"
if [ -f "$MLP" ]; then
...
#
# DISABLED: Mule is now directly installed with miniconda 
#
