...
if [ "${MULE_PLATFORM_ID:0:9}" == "supermuc_" ] || [ "${MULE_PLATFORM_ID:0:9}" == "linuxclus" ]; then
	if [ "$MULE_F90_COMPILER" == "intel" ]; then
		# Intel compiler pauses with -O3, hence we add -O2 to the FFLAGS
		echo "FFLAGS += -O2" >> Makefile.local

		# Specify module output path to be in ./include
		echo "FFLAGS += -module \"$(pwd)/include\"" >> Makefile.local
	fi
fi
