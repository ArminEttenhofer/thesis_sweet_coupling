...
Runtime parameters
"""
params_runtime_timestep_sizes = [960., 1920., 3840., 7680., 15360.] # [s]
total_simtime = 60 * 60 * 24 * 6 # 6 days

jg.runtime.benchmark_name = 'galewsky'
jg.parallelization.max_wallclock_seconds = 8*60*60 # 8 hours
...
ts_methods = ["SDC_Classic(lg,ADDT(lc,n))", "ETDSDC(lg,ADDT(lc,n))"]
# test orders
test_orders = [8, 12, 16]
...
if __name__ == "__main__":
    for tsm in ts_methods:
        ...
        if (tsm.find("ETDSDC") != -1):
            ...
            jg.runtime.timestepping_method = tsm
            for order in test_orders:
                SDCparams= getSDCSetup(nNodes=max(order, 2), nIter=order-1, nodeDistr="LEGENDRE", nodeType="LOBATTO")
