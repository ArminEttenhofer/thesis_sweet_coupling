...
ofile = open("etdsdc_imexsdc_high_gal_6_M128.txt", "w")
ofile.write("ETDSDC, IMEXSDC, Galewsky 6 days M128, phi pert\n")
...
for tagname_y in tagnames_y:
    ...
    for group_name, group_data in d.get_data_float().items():
        ...
        if order == 1:
            order = int(group_name[:2])

        if (group_name.find("ETDSDC") != -1):
            plt.loglog(group_data['x_values'], group_data['y_values'], '*:', mfc='w', markersize=8, color=colors[int(order/4+3)], label=group_name)
        else:
            plt.loglog(group_data['x_values'], group_data['y_values'], 'o:', mfc='w', markersize=6, color=colors[int(order/4+3)], label=group_name)
        prev_y = None
        prev_x = None
        conv = "----"
    ...
    figname = "conv_ETDSDC_IMEX_galewsky_6_128.png"
