...
def computeTopography(lon, lat):
    b = np.zeros((lon.size, lat.size))
    ...
    for i in range(lon.size):
        for j in range(lat.size):
            ln = lon[i] * np.pi / 180.
            lt = lat[j] * np.pi / 180.
            r = np.sqrt((ln - lon_c)**2 + (lt - lat_c)**2)
            if r <= R:
                b[i,j] = b0 * (1. - r / R)
...
extent_type = sys.argv[4];                  ## full domain or zoom (zoom_unstable_jet / zoom_unstable_jet_2)
compare_solution = int(sys.argv[5]);        ## 0: plot solution; 1: plot diff (sol - ref); 2: plot diff w.r.t. reference solution (Scott, Harris, Polvani, 2016)
filename_ref = sys.argv[6];                 ## path containing ref solution
roll = int(sys.argv[7])                     ## 1: sum 180^o on longitude
lim_ref = int(sys.argv[8])                  ## 0: limits of the colorbar are defined by solution; 1: limits of the colorbar legend are defined by the  ref solution
geopotential_elevation = int(sys.argv[9])   ## 1: add geopotential mean and topography to geopotential perturbation
input_lim = None
if len(sys.argv) > 10:
    input_lim = float(sys.argv[10]);        ## limits of colorbar: (-input_lim, input_lim)

assert geometry in ["plane", "sphere"]
assert plot_type in ["map", "contour", "both"]
assert extent_type in ["full", "zoom_unstable_jet", "zoom_unstable_jet_2"]
...
try:
    ...
    data_spec_ref = Sphere2DDataSpectral(filename_ref, setup_grid=True)
    data_ref = data_spec_ref.data
    ref_ok = True
except:
    ref_ok = False
...
for filename in filenames:
    ...
    data_spec = Sphere2DDataSpectral(filename, setup_grid=True)
    data = data_spec.data
    ...
    if geometry == "plane":
        ...
        x = data_spec.file_info["lons"] * 180. / np.pi;
        y = data_spec.file_info["lats"] * 180. / np.pi;
        x_ref = data_spec_ref.file_info["lons"] * 180. / np.pi;
        y_ref = data_spec_ref.file_info["lats"] * 180. / np.pi;

    if ('prog_h' in filename) and not compare_solution: ## define in km
        data = data / 1000;
    ...
    if ('prog_phi' in filename) and not compare_solution and geopotential_elevation: ## define in km
        data = data + g * 5960.
        data += computeTopography(x, y);
    ...
    if 'potvrt' in filename:
        ...
        data_spec_phi = Sphere2DDataSpectral(filename.replace("potvrt", "phi_pert"), setup_grid=True)
        data_phi = data_spec_phi.data + H * g
    ...
    if compare_solution == 1:
        if plot_type == "map" or plot_type == "both" or extent_type == "full" or ("prog_potvrt" not in filename):
            sys.exit()
        if "iter" in filename:
            time = float(filename.split("_t")[-1].split("_iter")[0]) / 24 / 3600
        else:
            time = float(filename.split("_t")[-1].split(".sweet")[0]) / 24 / 3600
        print("TIME", time)
        if time == 5:
    ...
    if geometry == "plane":
        ...
        if extent_type == "full":
            ...
            print("AAAAAAAAAAAAAAAAAA", x.shape, y.shape, data.shape)
            x = x[idx_x]
            y = y[idx_y]
            ...
            print("BBBBBBBBBBBBBBBBB", x.shape, y.shape, data.shape)
            if compare_solution != 1:
    ...
    if plot_type == "map" or plot_type == "both":
        ...
        if aspect == 'auto':
            ...
            cbar = plt.colorbar(fraction = 0.017, pad = 0.04, shrink=0.65)
    ...
    if compare_solution:
        outfilename = filename.replace('.sweet', '_diff.pdf')
    ...
    if compare_solution:
        ...
        outfilename = outfilename[:-6] + "_" + plot_type + outfilename[-6:];
        outfilename = outfilename[:-6] + "_" + extent_type + outfilename[-6:];

    if 'prog_phi_pert' in outfilename and geopotential_elevation:
        outfilename = outfilename.replace('prog_phi_pert', 'prog_phi_elevation')

    outfilename = outfilename.replace('.sweet', '.png')
