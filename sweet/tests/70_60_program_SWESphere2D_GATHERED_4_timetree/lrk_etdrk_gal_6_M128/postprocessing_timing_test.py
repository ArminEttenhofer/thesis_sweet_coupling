...
for tagname_y in tagnames_y:
    ...
    for group_name, group_data in d.get_data_float().items():
        ...
        if (group_name.find("LRK") != -1):
            plt.loglog(group_data['x_values'], group_data['y_values'], 'd:', mfc='w', markersize=8, color=colors[order-1], label=group_name)
        else:
            plt.loglog(group_data['x_values'], group_data['y_values'], 'o:', mfc='w', markersize=8, color=colors[order-1], label=group_name)
    ...
    figname = "time_LRK_ETDRK_galewsky_6_128.png"
