...
for i in vars_:
    tagnames_y += [
        f"cart2d_data_diff_prog_{i}.res_norm_l1",
        f"cart2d_data_diff_prog_{i}.res_norm_l2",
        f"cart2d_data_diff_prog_{i}.res_norm_linf"
    ]

    tag_cleanup_info += [
        {"ref_file_starts_with": f"output_prog_{i}", "tag_src": "res_norm_linf", "tag_dst": f"cart2d_data_diff_prog_{i}.res_norm_linf"},
        {"ref_file_starts_with": f"output_prog_{i}", "tag_src": "res_norm_l1", "tag_dst": f"cart2d_data_diff_prog_{i}.res_norm_l1"},
        {"ref_file_starts_with": f"output_prog_{i}", "tag_src": "res_norm_l2", "tag_dst": f"cart2d_data_diff_prog_{i}.res_norm_l2"}
    ]
...
JobsData_GroupsCleanupPostprocessed(job_groups, tag_cleanup_info, pickle_file_default_prefix="cart2d_data_norms_grid_space_", pint = True)
...
for key, group in job_groups.items():
    ...
    while True:
        ...
        for i in vars_:
            ...
            ## get offline errors
            try:
                err_offline['res_norm_linf'] = group[offline_job][f"cart2d_data_diff_prog_{i}.res_norm_linf" + niter_str]
                err_offline['res_norm_l1'] = group[offline_job][f"cart2d_data_diff_prog_{i}.res_norm_l1" + niter_str]
                err_offline['res_norm_l2'] = group[offline_job][f"cart2d_data_diff_prog_{i}.res_norm_l2" + niter_str]
            except:
                br = True
                break
            ...
            error_files = [f.replace("output_", "xbraid_error_fine_") for f in error_files if niter_str in f and f"prog_{i}" in f and "spec" not in f]

            path = group[online_job]['runtime.p_job_dirpath']
            for f in error_files:
                err_online['res_norm_l1'], err_online['res_norm_l2'], err_online['res_norm_linf'] = read_error_file(path + "/" + f)
