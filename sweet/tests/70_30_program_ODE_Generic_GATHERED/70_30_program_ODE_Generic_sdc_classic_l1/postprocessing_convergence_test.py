...
groups = ["runtime.timestepping_method", "output.sdc.idstring"]
...
for tagname_y in tagnames_y:
    ...
    if True:
        ...
        for group_name, group_data in d.get_data_float().items():
            ...
            if False:
                if "EXP" in group_name or "REXI" in group_name:
                    """
                    EXP / REXI are exact -> switch to error threshold test
                    """
                    test_type = "error"
                    error_tolerance_error = 1e-9
                ...
                if test_type == "convergence":
                    ...
                    if error_convergence != "-":
                        if error_convergence > error_tolerance_convergence:
                            if conv > target_conv:
                                print(f"[IGNORING] Computed convergence higher than desired one ({target_conv})")
                            elif y < 1e-11:
                                print(f"[IGNORING] Accuracy around machine precision ({1e-11})")
                            else:
                                print(f"Error: {error_convergence}")
                                raise Exception(f"Convergence exceeds tolerance of {error_tolerance_convergence}")
                    ...
                    if y > error_tolerance_error or math.isnan(y):
                        print("Error: "+str(y))
