...
jg.compile.sphere2d_spectral_space = 'disable'
jg.compile.sphere2d_spectral_dealiasing = 'disable'
...
#
jg.runtime.rexi_sphere2d_preallocation = 0
...
for group in groups:
    ...
    if group == 'ln1':
        ...
        ts_methods = [
                         "ERK(l,order=1)",
                         "SS(ERK(l,order=1),ERK(n,order=1),order=1)",
                         "SS(IRK(l,order=1),ERK(n,order=1),order=1)",
                         "SS(EXP(l,order=1),ERK(n,order=1),order=1)"
            ###['ln_erk',        1,    1],
            ###['l_erk_n_erk',        1,    1],
            ###['l_irk_n_erk',        1,    1],
            ###['l_rexi_n_erk',    1,    1],
    ...
    if group == 'ln2':
        ...
        ts_methods = [
                         ...
                         "SS(ERK(l,order=2),ERK(n,order=2),order=2)",
                         "SS(IRK(l,order=1),ERK(n,order=2),order=2)",
                         "ERK(l,order=2)",
                         "SS(EXP(l,order=2),ERK(n,order=2),order=2)"
            ###['l_cn_n_erk',        2,    2],
            ###['l_erk_n_erk',        2,    2],
            ###['l_irk_n_erk',        2,    2],
            ###['ln_erk',        2,    2],
    ...
    if group == 'ln2space':
        ...
        ts_methods = [
                         "ERK(l,order=2)",
                         ######### l_cn_na_sl_nd_settls
                         "SETTLS(l,na(sl_order=2),nr,order=2)",
                         ######### l_rexi_na_sl_nd_settls
                         #######"SETTLS(EXP(l),na(sl_order=2),nr,order=2)",
                         ######### l_rexi_na_sl_nd_etdrk
                         ###"SL(EXP(l),na(sl_order=2),nr,order=2)"
                         ###"ETDRK(EXP(l),n,order=2)"
                         #"SETTLS(lg,na(sl_order=2),ADDT(lc,nr),order=2)"
...
#            ['l_rexi_n_erk',    2,    2],
    ...
    for tsm in ts_methods:
        ...
        if group == 'ln2space' and 'ERK(ln' in tsm:
        ...
        for phys_res in phys_res_list:
            ...
            ###jg.runtime.timestepping_order = tsm[1]
            ###jg.runtime.timestepping_order2 = tsm[2]
            jg.runtime.space_res_grid = phys_res
