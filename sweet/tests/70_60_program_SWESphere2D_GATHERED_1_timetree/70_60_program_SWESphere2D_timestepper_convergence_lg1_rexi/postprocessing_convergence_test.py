...
groups = ['runtime.timestepping_method', 'compile.threading', 'compile.sweet_mpi', 'compile.rexi_thread_parallel_sum']
...
for tagname_y in tagnames_y:
    ...
    if True:
        ...
        for group_name, group_data in d.get_data_float().items():
            ...
            if 'phi' in tagname_y:
                if 'exp' in group_name.lower() or 'rexi' in group_name.lower():
                    test_type = 'error'
                    error_tolerance_error = 1e-5

            elif 'vrt' in tagname_y or 'div' in tagname_y:

                if 'exp' in group_name.lower() or 'rexi' in group_name.lower():
                    test_type = 'error'
                    error_tolerance_error = 1e-15

            elif 'div' in tagname_y:
                if 'exp' in group_name.lower() or 'rexi' in group_name.lower():
                    test_type = 'error'
