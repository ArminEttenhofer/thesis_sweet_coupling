...
# Deactivate threading to avoid thrashing of threads
...
jg.compile.program = "programs/xbraid_PDE_SWESphere2D"
jg.compile.mode = "debug"
jg.compile.sweet_mpi = "enable"
jg.compile.xbraid_sphere2d = 'enable'
...
jg.runtime.xbraid_spatial_coarsening = 1;
...
tsms_fine = [tsm_fine]
##tsms_coarse = ["SS(IRK(l,order=2),ERK(n,order=2),order=2)", "SETTLS(lg,na(sl_order=2),ADDT(lc,nr),order=2)", "SLETDRK(EXP(lg),na(sl_order=2),ADDT(lc,nr),order=2)"]
###tsms_coarse = ["SS(IRK(l,order=2),ERK(n,order=2),order=2)", "SETTLS(lg,na(sl_order=2),ADDT(lc,nr),order=2)", "ETDRK(EXP(lg),ADDT(lc,n),order=2)"]
