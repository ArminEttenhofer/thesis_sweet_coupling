...
class Plotting(InfoError):
    ...
    def plot_setup(
            ...
            plot_slopes = False,
            slopes = [1,2],
            slopes_position = [[0,100],[100,200]],
            slopes_scaling_factor = [1e-5, 1e-5],
            slopes_label = ["",""],
        ...
        self.plot_slopes = plot_slopes
        self.slopes = slopes
        self.slopes_position = slopes_position
        self.slopes_scaling_factor = slopes_scaling_factor
        self.slopes_label = slopes_label
    ...
    def plot_data(
        ...
        for key in sorted(data_plotting.keys()):
            ...
            if "plot_style" in data:
                plot_style.update(data["plot_style"])
            ...
            if self.plot_slopes:
                ...
                for islope in range(len(self.slopes)):

                    slope = self.slopes[islope]
                    y_ = np.power(self.slopes_position[islope], slope)*self.slopes_scaling_factor[islope]
                    line = plt.plot(self.slopes_position[islope], y_, linestyle="solid", color="gray")

                    x = line[0].get_xdata()[1]
                    y = line[0].get_ydata()[1]

                    self.ax.annotate(
                        self.slopes_label[islope],
                        xy=(x * 0.95, y * 1.35),
                        color=line[0].get_color(),
                        size=10,
                    )
