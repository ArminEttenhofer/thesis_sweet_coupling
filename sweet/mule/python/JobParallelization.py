...
class JobParallelization(InfoError):
    ...
    def __init__(self, dummy_init = False):
        ...
        # Use nested OpenMP num threads
        self.withNestedOpenMPNumThreads = False
...
#        # Support space-only parallelization without list
        ...
        if len(lThreads) > 0 and self.withNestedOpenMPNumThreads:
            self.omp_num_threads = ','.join([str(n) for n in lThreads])
        else:
            self.omp_num_threads = self.num_threads_per_rank
