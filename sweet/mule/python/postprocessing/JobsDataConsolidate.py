...
def JobsData_GroupsRegSub(job_groups, key_search_replace):
    """
    Cleanup job data by duplicating some values under new keys.

    For this, a regexp is used as given in key_search_replace.
    This is a list of tuples where the first element is the
    regexp search and the second one the replace.
    """
    ...
    for key, group in job_groups.items():
        ...
        for job_id, job_data in group.items():
            ...
            for key, value in job_data.items():
                for k in key_search_replace:
                    search = k[0]
                    replace = k[1]
                    key_new = re.sub(search, replace, key)
                    if key != key_new:
                        new_job_data[key_new] = value
...
class JobsData_GroupsPlottingScattered:
    ...
    def get_data_float(
        ...
        for key, value in data_plotting.items():
            x = []
            y = []
            for (i, j) in zip(value['x_values'], value['y_values']):
                if j != None:
                    x.append(float(i))
                    y.append(float(j))
