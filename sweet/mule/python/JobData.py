...
class JobData(InfoError):
    ...
    @property
    def _runtime(self):
        return self.__job_raw_data['jobgeneration']['runtime']

    @property
    def _compile(self):
        return self.__job_raw_data['jobgeneration']['compile']

    @property
    def output(self):
        return self.__job_raw_data['output']

    @property
    def path(self):
        return self._runtime['p_job_dirpath']

    def getOutputFiles(self, majorKey='TIME'):
        ext = '.sweet' if self._runtime['output_file_mode'] == 'bin' else '.csv'
        files = glob.glob(f'{self.path}/*{ext}')

        if self._compile['program'] == 'programs/PDE_SWESphere2D':
            fields = ['div', 'phi_pert', 'vrt']
        else:
            raise ValueError('outputFiles extraction not implemented for '
                             f'{self._compile["program"]}')

        timePattern = r'_t(\d+\.\d+)\.'+ext[1:]
        ...
        for fileName in files:
            for name in fields:
                if name in fileName:
                    sMatch = re.search(timePattern, fileName)
                    if sMatch is None:
                        raise ValueError(
                            f'output file {fileName} does not contain time tag')
                    time = float(sMatch.group(1))
                    try:
                        data[time][name] = fileName
                    except KeyError:
                        data[time] = {name: fileName}
                    times.add(time)

        self.outputTimes = sorted(times)
        data = {t: data[t] for t in self.outputTimes}

        if majorKey == 'TIME':
            return data
        elif majorKey == 'FIELD':
            return {n: {t: data[t][n] for t in self.outputTimes} for n in fields}
        else:
            raise ValueError(
                f'cannot get output files with majorKey={majorKey},'
                ' can only use TIME or FIELD')
