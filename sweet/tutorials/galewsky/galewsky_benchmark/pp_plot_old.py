...
def get_data_phys(filename):
    if filename.endswith(".sweet"):
        ...
        if 0:
            ...
            sphere2d_data = Sphere2DDataSpectral(filename, setup_grid=False)
            ops = SphereDataOperators(file_info=sphere2d_data.file_info)
            data_phys = ops.spec2phys(sphere2d_data.data_spectral)
