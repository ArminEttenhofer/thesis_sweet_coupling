...
class Sphere2DDataOperators:
    """
    Process data given in spectral space

    Usage case 1:

        from mule.postprocessing.Sphere2DDataOperators import *

        sphere2d_data = Sphere2DDataSpectral(input_file)
        ops = Sphere2DDataOperators(file_info=sphere2d_data.file_info)

        ... = ops...(sphere2d_data.data_spectral)


    Info: To just convert the data to grid space, use an option to Sphere2DDataSpectral:

        sphere2d_data = Sphere2DDataSpectral(input_file, setup_grid=True)
        data_phys = sphere2d_data.data_grid
    ...
    def __init__(
        ...
        """
        Setup from file_info data (originates from Sphere2DDataSpectral)
        """
    ...
    def setup_from_file_info(
        ...
        self.lap = self.lap/self.rsphere2d**2
        self.invlap = self.invlap*self.rsphere2d**2
    ...
    def vrtdiv2uv(self, vrtspec, divspec):
        return self._shtns.synth((self.invlap/self.rsphere2d)*vrtspec, (self.invlap/self.rsphere2d)*divspec)

    def uv2vrtdiv(self,u,v):
        vrtspec, divspec = self._shtns.analys(u, v)
        return self.lap*self.rsphere2d*vrtspec, self.lap*self.rsphere2d*divspec
