...
from mule.postprocessing.ODE_Generic_Data import *


class ODE_Generic_DataNorms:
    ...
    def compute_diff(
        ...
        file_a = ODE_Generic_Data(filename_a)
        file_b = ODE_Generic_Data(filename_b)
        ...
        self.N = len(file_a.data)

        for j in range(self.N):
            value = file_b.data[j]-file_a.data[j]

            # http://mathworld.wolfram.com/L1-Norm.html
            self.norm_l1_value += abs(value)
            # http://mathworld.wolfram.com/L2-Norm.html
            self.norm_l2_value += abs(value*np.conj(value))
            ...
            self.norm_rms_value += abs(value*np.conj(value))
