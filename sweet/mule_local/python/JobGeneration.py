...
import copy
import types
def module_deepcopy(module, memo):

    # Housekeeping
    cls = module.__class__
    new_module = cls.__new__(cls)
    memo[id(module)] = new_module

    #print(f"DC {module}")
    # Call __deepcopy__ if it exists
    if hasattr(module, '__deepcopy__'):
        return module.__deepcopy__()

    # Iterate over dict and link modules and functions
    for name, value in module.__dict__.items():
        #print(f"DCC {module} => {name}")
        if name.startswith('__'):
            setattr(new_module, name, copy.deepcopy(value))
        else:
            if isinstance(value, types.ModuleType) or isinstance(value, types.FunctionType):
...
copy._deepcopy_dispatch[types.ModuleType] = module_deepcopy
...
class JobGeneration(InfoError):
    ...
    def __init__(self, platform_id_override = None, dummy_init = False):
        ...
        # Whether postprocessing should be skipped for this benchmark
        # Useful for, e.g., plan generation
        ...
        # Is this a job which should be skipped in the postprocessing?
        self.skip_postprocessing = False
...
"""
    ...
    def get_compilecommands_accum(self):
        return self.p_compilecommands_accum
    ...
    def write_compilecommands(
            self,
            compilecommands_filename = None,
            content = None,
            content_accum = None
    ):
...
"""
        ...
        if content_accum is not None:
            ...
            content_output += self.get_compilecommands_content()
        ...
        script_file.write(content_output)
        script_file.close()
