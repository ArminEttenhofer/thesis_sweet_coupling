...
class JobCompileOptions(InfoError):
    ...
    def __init__(self, dummy_init = False):
        ...
        self.shacksCompile = mule.Shacks.Shacks("shacksCompile")

        for _ in self.shacksCompile.values():
            _.__init__(self)
        ...
        # Features
        self.cart2d_spectral_space = 'disable'
        self.cart2d_spectral_dealiasing = 'enable'
        self.sphere2d_spectral_space = 'disable'
        self.sphere2d_spectral_dealiasing = 'enable'
        self.libxml = 'disable'

        # GUI
        self.gui = 'disable'

        # Which parallel sdc parallelization model to use
    ...
    def getSConsParams(self):
        ...
        # Features
        retval += ' --cart2d-spectral-space='+self.cart2d_spectral_space
        retval += ' --cart2d-spectral-dealiasing='+self.cart2d_spectral_dealiasing
        retval += ' --sphere2d-spectral-space='+self.sphere2d_spectral_space
        retval += ' --sphere2d-spectral-dealiasing='+self.sphere2d_spectral_dealiasing
    ...
    def postprocessOptions(self):
         ...
         * MULE_COMPILE_FILES_AND_DIRS: src/programs/swe_sphere2d/
         * MULE_COMPILE_FILES_AND_DIRS: src/include/benchmarks_sphere2d_swe/
        ...
        if self.cart2d_spectral_space == 'enable':
            self.libfft = 'enable'
        else:
            self.cart2d_spectral_dealiasing = 'disable'

        if self.sphere2d_spectral_space == 'enable':
            self.libsph = 'enable'
        else:
            self.sphere2d_spectral_dealiasing = 'disable'
    ...
    def sconsAddOptions(self, scons):
        ...
        scons.AddOption(    '--cart2d-spectral-space',
                dest='cart2d_spectral_space',
                type='choice',
                choices=['enable', 'disable'],
                default='enable',
                help="Activate spectral space for data on the cart2d (2D FFT) [default: %default]"
        )
        self.cart2d_spectral_space = scons.GetOption('cart2d_spectral_space')

        scons.AddOption(    '--sphere2d-spectral-space',
                dest='sphere2d_spectral_space',
                type='choice',
                choices=['enable', 'disable'],
                default='disable',
                help="Activate spectral space for data on the sphere2d (Spherical Harmonics) [default: %default]"
        )
        self.sphere2d_spectral_space = scons.GetOption('sphere2d_spectral_space')
        ...
        scons.AddOption(    '--cart2d-spectral-dealiasing',
                dest='cart2d_spectral_dealiasing',
                type='choice',
                choices=['enable','disable'],
                default='enable',
                help='spectral dealiasing (3N/2-1 rule): enable, disable [default: %default]'
        )
        self.cart2d_spectral_dealiasing = scons.GetOption('cart2d_spectral_dealiasing')

        scons.AddOption(    '--sphere2d-spectral-dealiasing',
                dest='sphere2d_spectral_dealiasing',
                type='choice',
                choices=['enable','disable'],
                default='enable',
                help='spectral dealiasing (3N/2-1 rule): enable, disable [default: %default]'
        )
        self.sphere2d_spectral_dealiasing = scons.GetOption('sphere2d_spectral_dealiasing')

        # Always use dealiasing for sphere2d
        if self.sphere2d_spectral_space == 'enable' and self.sphere2d_spectral_dealiasing != 'enable':
            raise Exception("self.sphere2d_spectral_dealiasing != enable")
    ...
    def getUniqueID(self, i_filter_list = []):
        ...
        if not 'compile_misc' in i_filter_list:
            ...
            if not 'compile_cart2d' in i_filter_list:
                if self.cart2d_spectral_space == 'enable':
                    retval+='_plspec'

                    if self.cart2d_spectral_dealiasing == 'enable':
                        retval+='_pldeal'

            if not 'compile_sphere2d' in i_filter_list:
                if self.sphere2d_spectral_space == 'enable':
                    retval+='_spspec'

                    if self.sphere2d_spectral_dealiasing == 'enable':
                        retval+='_spdeal'

            if self.gui == 'enable':
                retval+='_gui'
                
            if self.parallel_sdc_par_model == ['omp', None]:
                retval+='_psdcpmomp'
