...
def CompileSWECart2D(p):
    p.compile.program = 'swe_cart2d'
    p.compile.cart2d_spectral_space = 'enable'
    #p.compile.cart2d_spectral_dealiasing = 'enable'
    p.compile.cart2d_spectral_dealiasing = 'disable'
    p.compile.sphere2d_spectral_space = 'disable'
    p.compile.sphere2d_spectral_dealiasing = 'disable'
    p.compile.threading = 'omp'
    p.compile.quadmath = 'disable'
    return p


def RuntimeSWECart2DNondimParameters(p):
    p.runtime.gravitation= 1
    p.runtime.sphere2d_rotating_coriolis_omega = 1
    p.runtime.h0 = 1
    p.runtime.cart2d_domain_size = 1
    return p

def RuntimeSWECart2DEarthParam(p):
    s = EarthMKSDimensions()
    p.runtime.gravitation= s.g
    p.runtime.sphere2d_rotating_coriolis_omega = s.f
    p.runtime.h0 = 10000
    p.runtime.cart2d_domain_size = 2.0*math.pi*s.erad # 40031555.8928087
    return p

def RuntimeSWECart2DEarthParamApprox(p):
    s = EarthMKSDimensionsApprox()
    p.runtime.gravitation= s.g
    p.runtime.sphere2d_rotating_coriolis_omega = s.f
    p.runtime.h0 = 10000
    p.runtime.cart2d_domain_size = 40000000 #2.0*math.pi*s.erad # 40031555.8928087
    return p
...
def SetupFDCMethods(p):
    ...
    #p.compile.cart2d_spectral_space = 'disable'
    p.compile.cart2d_spectral_dealiasing = 'disable'
