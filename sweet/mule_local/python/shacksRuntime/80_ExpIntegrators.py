...
class ExpIntegrators:
    def __init__(self):
        ...
        self.rexi_sphere2d_preallocation = 0
    ...
    def load_from_dict(self, d):
        ...
        if 'sphere2d_preallocation' in d:
            self.rexi_sphere2d_preallocation = d['sphere2d_preallocation']
    ...
    def getUniqueID(self,
        ...
        if 'runtime.rexi' in filter_list:
            return idstr

        if self.rexi_method is not None and self.rexi_method != "":
            if self.rexi_method == 'direct':
                ...
                if self.rexi_method == "file":
                    if len(self.rexi_files_coefficients) > 0:
                        ...
                        if not 'runtime.rexi_params' in filter_list:
                            ...
                            for i in range(len(self.rexi_files_coefficients)):
                                ...
                                if not 'runtime.rexi_params_'+r.function_name in filter_list:
                                    idstr += "_"+r.getUniqueId()
                    ...
                    if not 'runtime.rexi_params' in filter_list:
                        ...
                        idstr += '_n'+str(self.rexi_normalization)
    ...
    def getRuntimeOptions(self):
        ...
        if self.rexi_method is not None and self.rexi_method != "":
            ...
            if self.rexi_method == 'direct':
                ...
                retval += ' --rexi-sphere2d-preallocation='+str(self.rexi_sphere2d_preallocation)
                ...
                if self.rexi_method == 'file':
                    ...
                    for coeffs in self.rexi_files_coefficients:
                        ...
                        if coeff_filepath in file_paths:
                            raise Exception(f"Path '{coeff_filepath}' already used")
                        
                        coeffs.write_file(coeff_filepath)
                        file_params.append(coeffs.function_name+':'+coeff_filepath)
                        
                        file_paths.append(coeff_filepath)
                    ...
                    if self.rexi_ci_max_real is not None:
                        retval += ' --rexi-ci-max-real='+str(self.rexi_ci_max_real)
                        if self.rexi_ci_max_imag is not None:
                            retval += ' --rexi-ci-max-imag='+str(self.rexi_ci_max_imag)
                    else:
                        if self.rexi_ci_sx is not None:
                            retval += ' --rexi-ci-sx='+str(self.rexi_ci_sx)
                        if self.rexi_ci_sy is not None:
                            retval += ' --rexi-ci-sy='+str(self.rexi_ci_sy)
                        if self.rexi_ci_mu is not None:
                            retval += ' --rexi-ci-mu='+str(self.rexi_ci_mu)

                else:
                    raise Exception(f"Unknown REXI method '{self.rexi_method}'")

                if self.rexi_normalization is not None:
