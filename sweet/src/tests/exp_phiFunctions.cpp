...
int main()
{
	...
	sweet::ExpIntegration::ExpFunction<T> fun;
	fun.setup("phi0");
	...
	for (int phi_or_ups_n = 0; phi_or_ups_n < 7; phi_or_ups_n++)
	{
		for (double x_re = -2; x_re < 2; x_re+=d_re)
		{
			...
			for (double x_im = -2; x_im < 2; x_im+=d_im)
			{
				...
				if (phi_or_ups_n < 4)
				{
					...
					err = std::abs(y - fun.phiN(phi_or_ups_n, x));
				}
				else
				{
					y = fun.upsN(phi_or_ups_n-3, x);
					err = std::abs(y - fun.upsN(phi_or_ups_n-3, x));
				}
