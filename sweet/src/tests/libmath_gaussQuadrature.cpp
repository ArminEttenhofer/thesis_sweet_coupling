...
)
{
	...
	{
		double quad_val = sweet::LibMath::GaussQuadrature::integrate5_intervals_adaptive_linear<double>(int_start, int_end, fun, error_threshold);
		...
		if (std::abs(quad_val - int_value) > error_threshold)
			SWEETErrorFatal("ERROR threshold exceeded");
	}


	{
		double quad_val = sweet::LibMath::GaussQuadrature::integrate5_intervals_adaptive_recursive<double>(int_start, int_end, fun, error_threshold);
	...
	{
		double quad_val = sweet::LibMath::GaussQuadrature::integrate5_intervals<double>(int_start, int_end, fun, 3);
