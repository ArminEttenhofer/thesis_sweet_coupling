...
)
{
	...
	if (error > eps)
	{
		...
		SWEETErrorFatal("Error too large!");
	}
}
...
int main(int i_argc, char *i_argv[])
{
...

	{
		...
		std::cout << "* Used functions: latlon_2_cartesian, cartesian_2_latlon" << std::endl;
		std::cout << "************************************************************" << std::endl;
		for (double a_lon = dlon*0.5; a_lon < 2.0*M_PI; a_lon += 0.3)
		{
			...
			double a_x, a_y, a_z;
			sweet::LibMath::VectorMath::point_latlon_2_cartesian__scalar(a_lon, a_lat, a_x, a_y, a_z);

			double o_lon, o_lat;
			sweet::LibMath::VectorMath::point_cartesian_2_latlon__scalar(a_x, a_y, a_z, o_lon, o_lat);
...

	{
		...
		std::cout << "************************************************************" << std::endl;
		{
			...
			for (double a_lon = 0; a_lon < 2.0*M_PI; a_lon += 0.3)
			{
				...
				sweet::LibMath::VectorMath::velocity_latlon_2_cartesian__scalar(a_lon, a_lat, vel_u, vel_v, vel_x, vel_y, vel_z);
...

	{
		...
		int k = 0;
		{
			...
			sweet::LibMath::VectorMath::point_latlon_normalize__scalar(a_lon, a_lat);
...

	{
		...
		std::cout << "* Tested functions: latlon_2_cartesian, cartesian_2_latlon" << std::endl;
		std::cout << "************************************************************" << std::endl;
...

	{
		...
		std::cout << "* Tested functions: velocity_latlon_2_cartesian_scalar, velocity_cartesian_2_latlon_scalar" << std::endl;
		std::cout << "************************************************************" << std::endl;
			...
			for (double a_lat = -M_PI*0.5+dlat*0.5; a_lat <= M_PI*0.5; a_lat += dlat)
			{
				for (double a_u = -2.0; a_u <= 2.0; a_u += 0.5)
				{
					for (double a_v = -2.0; a_v <= 2.0; a_v += 0.25)
					{
						...
						sweet::LibMath::VectorMath::velocity_latlon_2_cartesian__scalar(a_lon, a_lat, a_u, a_v, a_V_x, a_V_y, a_V_z);

						double o_u, o_v;
						sweet::LibMath::VectorMath::velocity_cartesian_2_latlon__scalar(a_lon, a_lat, a_V_x, a_V_y, a_V_z, o_u, o_v);
...

	{
		...
		for (int i = 0; i < num_test_cases; i++)
		{
			...
			sweet::LibMath::VectorMath::point_rotate_3d__scalar(
					testcases[i][0],	// start
					testcases[i][1],
					testcases[i][2],
					testcases[i][3],	// angle
