 ...
 *
 * Based on previous normal mode implementation by Martin Schreiber in swe_cart2d.cpp
 */

#ifndef TESTS_PDE_SWECART2D_NORMALMODECONVERSION_SWECART2DNORMALMODES_HPP
#define TESTS_PDE_SWECART2D_NORMALMODECONVERSION_SWECART2DNORMALMODES_HPP
...
/*Debug output*/
	...
	)
	{
		...
		SWECart2DNormalModes::sw_eigen_decomp(
				k0,				//wavenumber in x
				k1,				// wavenumeber in y
	...
	)
	{
		...
		else
		{
			...
			sprintf(
					...
					shackTimestepControl->currentTimestepSize*shackIOData->outputFormatTimeScale
				);
			std::ofstream file(buffer_real, std::ios_base::trunc);
			...
			if (number_of_prognostic_variables <= 0)
				SWEETErrorFatal("simVars.pde.number_of_prognostic_variables must be set!");
...
#if 0
			if (shackCart2DDataOps->timestepping_method == SimulationVariables::Discretization::LEAPFROG_EXPLICIT)
			{
				...
				shackTimestepControl->currentTimestepSize = 0.5*shackPDESWECart2D->CFL;
				shackPDESWECart2D->CFL = -shackTimestepControl->currentTimestepSize;
			}
#endif
...
//			file << "# r " << shackSWECart2D->sphere2d_radius << std::endl;
			...
			for (int outer_prog_id = 0; outer_prog_id < number_of_prognostic_variables; outer_prog_id++)
			{
				...
				{
					Cart2DDataComplex t1(cart2DDataConfig);
					Cart2DDataComplex t2(cart2DDataConfig);
					Cart2DDataComplex t3(cart2DDataConfig);
					...
					for (std::size_t outer_i = 0; outer_i < cart2DDataConfig->spectral_complex_array_data_number_of_elements; outer_i++)
					{
						...
						// convert Cart2DDataComplex to Cart2DData
						for (int inner_prog_id = 0; inner_prog_id < number_of_prognostic_variables; inner_prog_id++)
						{
							*prog[inner_prog_id] = Convert_Cart2DDataComplex_2_Cart2DData::grid_convert(*prog_cplx[inner_prog_id]);
