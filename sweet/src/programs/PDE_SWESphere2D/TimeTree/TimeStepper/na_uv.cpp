...
const std::vector<std::string> na_uv::getNodeNames()
{
...
bool na_uv::outputHelp(
		std::ostream &o_ostream,
		const std::string &i_prefix
)
{
	o_ostream << i_prefix << "InteriorNode: 'Non-linear advection term (using velocity-based formulation)':" << std::endl;
...
bool na_uv::setupConfigAndForwardTimeStepperEval(
...
)
{
	...
	else if (_evalTypeRequested == EVAL_SEMI_LAGRANGIAN)
	{
		if (_semiLagrangian.order != 1 && _semiLagrangian.order != 2)
		{
			...
			oss << "na_uv DE term: Invalid time stepping order " << _semiLagrangian.order;
...
bool na_uv::setupTreeNodeByFunction(
...
bool na_uv::evalNA_getNumStates(
		int *o_numStates
)
{
	*o_numStates = 2;
...
bool na_uv::evalNA_departurePoints(
...
)
{
	...
	_semiLagrangian.helper->apply_sl_timeintegration_uv(
				_ops,
