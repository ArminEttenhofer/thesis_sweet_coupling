#ifndef PROGRAMS_PDE_SWESPHERE2D_DATACONTAINER_SIMULATION_HPP
#define PROGRAMS_PDE_SWESPHERE2D_DATACONTAINER_SIMULATION_HPP
...
public:
	...
	std::vector<std::string> var_names = {"prog_phi_pert", "prog_vrt", "prog_div"};

	sweet::Data::Sphere2D::DataSpectral data[N];

	// Note, that these references don't increase the size of the class
	sweet::Data::Sphere2D::DataSpectral& phi_pert = data[0];
	sweet::Data::Sphere2D::DataSpectral& vrt = data[1];
	sweet::Data::Sphere2D::DataSpectral& div = data[2];
...
public:
	...
	double reduceNormL1Grid(int i, bool normalized = false) const override
	{
		...
		if (normalized)
		{
			std::size_t resx_data = data[i].sphere2DDataConfig->grid_num_lon;
			std::size_t resy_data = data[i].sphere2DDataConfig->grid_num_lat;
	...
	{
		int s = data[0].sphere2DDataConfig->spectral_array_data_number_of_elements;
		for (int i = 0; i < N; i++)
...
public:
	...
	{
		if (i_filename.find("prog_phi_pert") != std::string::npos)
			phi_pert.loadDataFromFile(i_filename, i_filemode);
		else if (i_filename.find("prog_vrt") != std::string::npos)
			vrt.loadDataFromFile(i_filename, i_filemode);
		else if (i_filename.find("prog_div") != std::string::npos)
			div.loadDataFromFile(i_filename, i_filemode);
		else
