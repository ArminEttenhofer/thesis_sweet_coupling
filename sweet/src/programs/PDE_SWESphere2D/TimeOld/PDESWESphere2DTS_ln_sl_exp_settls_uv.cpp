...
bool PDESWESphere2DTS_ln_sl_exp_settls_uv::implementsTimesteppingMethod(const std::string &i_timestepping_method)
{
	/*
...
bool PDESWESphere2DTS_ln_sl_exp_settls_uv::setup_auto(
...
)
{
	...
	PDESWESphere2DTS_ln_sl_exp_settls_uv::LinearCoriolisTreatment_enum _linear_coriolis_treatment = PDESWESphere2DTS_ln_sl_exp_settls_uv::CORIOLIS_IGNORE;
	PDESWESphere2DTS_ln_sl_exp_settls_uv::NLRemainderTreatment_enum _nonlinear_remainder_treatment = PDESWESphere2DTS_ln_sl_exp_settls_uv::NL_REMAINDER_IGNORE;
	...
	if (timestepping_method.find("l_exp") != std::string::npos)
		_linear_coriolis_treatment = PDESWESphere2DTS_ln_sl_exp_settls_uv::CORIOLIS_LINEAR;
	else if (timestepping_method.find("lc_na_sl") != std::string::npos)
		_linear_coriolis_treatment = PDESWESphere2DTS_ln_sl_exp_settls_uv::CORIOLIS_SEMILAGRANGIAN;
	else if (timestepping_method.find("lc_") != std::string::npos)
		_linear_coriolis_treatment = PDESWESphere2DTS_ln_sl_exp_settls_uv::CORIOLIS_NONLINEAR;


	// Search for nonlinear remainder term
	if (timestepping_method.find("_nr_") != std::string::npos)
		_nonlinear_remainder_treatment = PDESWESphere2DTS_ln_sl_exp_settls_uv::NL_REMAINDER_NONLINEAR;
	...
	if (_linear_coriolis_treatment == PDESWESphere2DTS_ln_sl_exp_settls_uv::CORIOLIS_LINEAR)
		string_id_storage += "l";
	else
	...
	if (_linear_coriolis_treatment == PDESWESphere2DTS_ln_sl_exp_settls_uv::CORIOLIS_SEMILAGRANGIAN)
	...
	if (_linear_coriolis_treatment == PDESWESphere2DTS_ln_sl_exp_settls_uv::CORIOLIS_NONLINEAR)
		string_id_storage += "_lc";

	if (_nonlinear_remainder_treatment == PDESWESphere2DTS_ln_sl_exp_settls_uv::NL_REMAINDER_NONLINEAR)
...
bool PDESWESphere2DTS_ln_sl_exp_settls_uv::setup_main(
...
)
{
	...
	swe_sphere2d_ts_l_rexi.setup_variant_50(
			io_ops,
			shackExpIntegration,
			"phi0",
...
std::string PDESWESphere2DTS_ln_sl_exp_settls_uv::getIDString()
{
	return string_id_storage;
}


void PDESWESphere2DTS_ln_sl_exp_settls_uv::runTimestep(
...
)
{
	...
	SWEETErrorFatal("Only 2nd order TS method supported!");
}



void PDESWESphere2DTS_ln_sl_exp_settls_uv::run_timestep_2nd_order(
...
)
{
	 ...
	 */
	{
		 ...
		 * exp(dtL) N(t-dt)
		 */
		swe_sphere2d_ts_l_rexi.runTimestep(
			N_U_phi_prev_nr, N_U_vrt_prev_nr, N_U_div_prev_nr,
			i_dt,
...
void PDESWESphere2DTS_ln_sl_exp_settls_uv::printHelp()
{
...
PDESWESphere2DTS_ln_sl_exp_settls_uv::PDESWESphere2DTS_ln_sl_exp_settls_uv()
{
}
