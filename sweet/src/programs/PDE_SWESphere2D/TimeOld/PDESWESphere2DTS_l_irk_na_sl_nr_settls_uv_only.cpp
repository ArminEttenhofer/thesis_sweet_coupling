...
bool PDESWESphere2DTS_l_irk_na_sl_nr_settls_uv_only::implementsTimesteppingMethod(const std::string &i_timestepping_method)
{
...
std::string PDESWESphere2DTS_l_irk_na_sl_nr_settls_uv_only::getIDString()
{
	return "l_irk_na_sl_nr_settls_uv_only";
}


bool PDESWESphere2DTS_l_irk_na_sl_nr_settls_uv_only::setup_auto(
...
bool PDESWESphere2DTS_l_irk_na_sl_nr_settls_uv_only::setup_main(
...
)
{
	...
	swe_sphere2d_ts_ln_erk_split_uv__l_erk_1st_order.setup_main(ops, 1, true, true, false, false, false);

	// Initialize with 1st order and half time step size
	swe_sphere2d_ts_l_irk.setup_main(ops, 1, 0.5 * shackTimestepControl->currentTimestepSize, shackPDESWETimeDisc->timestepping_crank_nicolson_filter, false);
...
void PDESWESphere2DTS_l_irk_na_sl_nr_settls_uv_only::runTimestep(
...
void PDESWESphere2DTS_l_irk_na_sl_nr_settls_uv_only::run_timestep_2nd_order_pert(
...
{
	if (i_dt <= 0)
		SWEETErrorFatal("PDESWESphere2DTS_l_na_settls_only: Only constant time step size allowed (Please set --dt)");
	 ...
	 */
	swe_sphere2d_ts_ln_erk_split_uv__l_erk_1st_order.euler_timestep_update_lg(
			U_phi, U_vrt, U_div,
	 ...
	 */
	swe_sphere2d_ts_ln_erk_split_uv__l_erk_1st_order.euler_timestep_update_lc(
			U_phi, U_vrt, U_div,
			L_U_phi, L_U_vrt, L_U_div,
			i_simulation_timestamp
		);
	...
	if (1)
	{
		...
		swe_sphere2d_ts_ln_erk_split_uv__l_erk_1st_order.euler_timestep_update_nr(
...
PDESWESphere2DTS_l_irk_na_sl_nr_settls_uv_only::PDESWESphere2DTS_l_irk_na_sl_nr_settls_uv_only()
{
}
