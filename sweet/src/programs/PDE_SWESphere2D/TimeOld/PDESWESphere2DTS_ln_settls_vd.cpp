...
bool PDESWESphere2DTS_ln_settls_vd::setup_auto(
...
)
{
	...
	PDESWESphere2DTS_ln_settls_vd::LinearCoriolisTreatment_enum _linear_coriolis_treatment = PDESWESphere2DTS_ln_settls_vd::CORIOLIS_IGNORE;
	PDESWESphere2DTS_ln_settls_vd::NLRemainderTreatment_enum _nonlinear_divergence_treatment = PDESWESphere2DTS_ln_settls_vd::NL_REMAINDER_IGNORE;
	bool _original_linear_operator_sl_treatment = true;

	if (timestepping_method == "ln_settls")
	{
		_linear_coriolis_treatment = PDESWESphere2DTS_ln_settls_vd::CORIOLIS_LINEAR;
		_nonlinear_divergence_treatment = PDESWESphere2DTS_ln_settls_vd::NL_REMAINDER_NONLINEAR;
	...
	else
	{
		...
		if (timestepping_method.find("l_irk") != std::string::npos || timestepping_method.find("l_exp") != std::string::npos)
			...
			_linear_coriolis_treatment = PDESWESphere2DTS_ln_settls_vd::CORIOLIS_SEMILAGRANGIAN;
		else if (timestepping_method.find("lc_") != std::string::npos)
			_linear_coriolis_treatment = PDESWESphere2DTS_ln_settls_vd::CORIOLIS_NONLINEAR;
		...
		if (_linear_coriolis_treatment == PDESWESphere2DTS_ln_settls_vd::CORIOLIS_LINEAR)
			string_id_storage += "l";
		else
		...
		if (_linear_coriolis_treatment == PDESWESphere2DTS_ln_settls_vd::CORIOLIS_SEMILAGRANGIAN)
		...
		if (_linear_coriolis_treatment == PDESWESphere2DTS_ln_settls_vd::CORIOLIS_NONLINEAR)
			string_id_storage += "_lc";

		if (_nonlinear_divergence_treatment == PDESWESphere2DTS_ln_settls_vd::NL_REMAINDER_NONLINEAR)
...
bool PDESWESphere2DTS_ln_settls_vd::setup_main(
...
bool PDESWESphere2DTS_ln_settls_vd::implementsTimesteppingMethod(const std::string &i_timestepping_method)
{
	/*
...
std::string PDESWESphere2DTS_ln_settls_vd::getIDString()
{
	return string_id_storage;
}


void PDESWESphere2DTS_ln_settls_vd::runTimestep(
...
void PDESWESphere2DTS_ln_settls_vd::run_timestep_2nd_order(
...
)
{
	...
	if (original_linear_operator_sl_treatment)
	{
		...
		if (coriolis_treatment == CORIOLIS_SEMILAGRANGIAN)
		{
			swe_sphere2d_ts_ln_erk_split_vd.euler_timestep_update_lg(
...
PDESWESphere2DTS_ln_settls_vd::PDESWESphere2DTS_ln_settls_vd()
{
}
