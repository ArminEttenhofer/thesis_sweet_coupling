...
#ifndef PROGRAMS_PDE_SWESPHERE2D_TIMEHELPERS_SPHBANDEDMATRIX_GRIDCOMPLEX_HPP
#define PROGRAMS_PDE_SWESPHERE2D_TIMEHELPERS_SPHBANDEDMATRIX_GRIDCOMPLEX_HPP

#include <sweet/Data/Sphere2D/Helpers/Helpers_SPHIdentities.hpp>
#include <sweet/LibMath/BandedMatrixGridComplex.hpp>
 ...
 */
class SphBandedMatrix_GridComplex	:
...
public:
	 ...
	 */
	sweet::LibMath::BandedMatrixGridComplex<T> lhs;

	/*!
	 * Buffer
	 */
	sweet::LibMath::BandedMatrixGridComplex<T> lhsBuffer;
	 ...
	 */
	sweet::LibMath::BandedMatrixSolverComplex bandedMatrixSolver;


	/*!
	 * Buffer to compactify the N-varying for a particular M
	 */
	std::vector<std::complex<double>> buffer_in, buffer_out;
...
public:
	...
	)
	{
		...
		buffer_in.resize(sphere2DDataConfig->spectral_complex_array_data_number_of_elements);
		buffer_out.resize(sphere2DDataConfig->spectral_complex_array_data_number_of_elements);
	 ...
	 */
	sweet::Data::Sphere2DComplex::DataSpectral apply(
			const sweet::Data::Sphere2DComplex::DataSpectral &i_x	//!< solution to be searched
	)
	{
		SWEET_ASSERT(isSetup);
		
		sweet::Data::Sphere2DComplex::DataSpectral out(sphere2DDataConfig);
	...
	sweet::Data::Sphere2DComplex::DataSpectral solve(
			const sweet::Data::Sphere2DComplex::DataSpectral &i_rhs
	)
	{
		sweet::Data::Sphere2DComplex::DataSpectral out(sphere2DDataConfig);

		i_rhs.check_sphere2DDataConfig_identical_res(sphere2DDataConfig);

		SWEET_THREADING_SPACE_PARALLEL_FOR
		for (int m = -sphere2DDataConfig->spectral_modes_m_max; m <= sphere2DDataConfig->spectral_modes_m_max; m++)
		{
			int idx = sphere2DDataConfig->getArrayIndexByModes_Complex_NCompact(std::abs(m),m);

#if 1
			int num_rows = sphere2DDataConfig->spectral_modes_n_max+1-std::abs(m);
			...
			/*
			 * Coefficients in with complex physical values are stored differently
			 * and we first need to store them compactly in a buffer array
			 */
			for (int n = std::abs(m); n <= sphere2DDataConfig->spectral_modes_n_max; n++)
			{
				int buffer_idx = n-std::abs(m);
				SWEET_ASSERT(buffer_idx < sphere2DDataConfig->spectral_modes_n_max+1);
				buffer_in[idx + buffer_idx] = i_rhs.spectral_space_data[sphere2DDataConfig->getArrayIndexByModes_Complex(n,m)];
			}
			...
			for (int n = std::abs(m); n <= sphere2DDataConfig->spectral_modes_n_max; n++)
			{
				int buffer_idx = n-std::abs(m);
				SWEET_ASSERT(buffer_idx < sphere2DDataConfig->spectral_modes_n_max+1);
				out.spectral_space_data[sphere2DDataConfig->getArrayIndexByModes_Complex(n,m)] = buffer_in[idx + buffer_idx];
			}
			 ...
			 */
			{
				...
				{
					SWEET_ASSERT(buffer_idx < sphere2DDataConfig->spectral_modes_n_max+1);
					buffer_in[buffer_idx] = i_rhs.spectral_space_data[sphere2DDataConfig->getArrayIndexByModes_Complex(n,m)];
					buffer_idx++;
				}
			}
			...
			bandedMatrixSolver.solve_diagBandedInverse_Carray(
							...
							buffer_in,
							buffer_out,
							sphere2DDataConfig->spectral_modes_n_max+1-std::abs(m),	// size of block (same as for SPHSolver)
							idx
					);
			 ...
			 */
			{
				...
				for (int n = std::abs(m); n <= sphere2DDataConfig->spectral_modes_n_max; n++)
				{
					out.spectral_space_data[sphere2DDataConfig->getArrayIndexByModes_Complex(n,m)] = buffer_out[buffer_idx];
					buffer_idx++;
				}
			}
#endif
