...
#ifndef PROGRAMS_PDE_ADVECTIONSPHERE2D_BENCHMARKS_PDEADVECTIONSPHERE2DBENCHMARK_NAIR_LAURITZEN_SL_HPP
#define PROGRAMS_PDE_ADVECTIONSPHERE2D_BENCHMARKS_PDEADVECTIONSPHERE2DBENCHMARK_NAIR_LAURITZEN_SL_HPP
...
class PDEAdvectionSphere2DBenchmark_nair_lauritzen_sl	:
		public PDEAdvectionSphere2DBenchmarks_BaseInterface
{
public:
	PDEAdvectionSphere2DBenchmark_nair_lauritzen_sl()
	{
	}
	...
	)
	{
		 ...
		 * test cases for linear transport problems on the sphere2D"
		 */
		...
		//double sphere2d_radius = 6.37122e6;
		double h0 = 1.0;				// h_max

		if (std::isinf(shackTimestepControl->maxSimulationTime))
			shackTimestepControl->maxSimulationTime = 12*24*60*60;		// default: 12 days

		// update operators
		ops->setup(ops->sphere2DDataConfig, shackSphere2DDataOps);
		 ...
		 * (Section 3.1.1)
		 */
		sweet::Data::Sphere2D::DataGrid phi_pert_phys_1(sphere2DDataConfig);
		{
			// Cosine bells

			sweet::Data::Sphere2D::DataGrid phi_pert_phys(sphere2DDataConfig);
			phi_pert_phys.grid_update_lambda(
		 ...
		 * (Section 3.1.2)
		 */
		sweet::Data::Sphere2D::DataGrid phi_pert_phys_1(ops->sphere2DDataConfig);
		{
			// Bump 1

			// Caption Figure 1
			double x0[3];
			sweet::LibMath::VectorMath::point_latlon_2_cartesian__scalar(i_lambda0, i_theta0, x0[0], x0[1], x0[2]);

			phi_pert_phys_1.grid_update_lambda(
				[&](double i_lambda, double i_theta, double &io_data)
				{
					double x[3];
					sweet::LibMath::VectorMath::point_latlon_2_cartesian__scalar(i_lambda, i_theta, x[0], x[1], x[2]);
		...
		sweet::Data::Sphere2D::DataGrid phi_pert_phys_2(ops->sphere2DDataConfig);
		{
			// Bump 2

			// Caption Figure 1
			double x0[3];
			sweet::LibMath::VectorMath::point_latlon_2_cartesian__scalar(i_lambda1, i_theta1, x0[0], x0[1], x0[2]);

			phi_pert_phys_2.grid_update_lambda(
				[&](double i_lambda, double i_theta, double &io_data)
				{
					double x[3];
					sweet::LibMath::VectorMath::point_latlon_2_cartesian__scalar(i_lambda, i_theta, x[0], x[1], x[2]);
		...
		o_phi_pert.loadSphere2DDataGrid(phi_pert_phys_1 + phi_pert_phys_2);
#endif
	...
	void get_varying_velocities(
			sweet::Data::Sphere2D::DataGrid &o_u_phys,
			sweet::Data::Sphere2D::DataGrid &o_v_phys,
	...
	)
	{
		...
		if (benchmark_name == "nair_lauritzen_case_test")
		{
			...
			double T = shackTimestepControl->maxSimulationTime;

			// velocity
			double u0 = 2.0*M_PI*shackSphere2DDataOps->sphere2d_radius/T;

			// we set k to 2.4 (p. 5)
			double k = 0.5;

			o_u_phys.grid_update_lambda(
				[&](double i_lambda, double i_theta, double &io_data)
				{
					// time varying flow
		...
		if (benchmark_name == "nair_lauritzen_case_1")
		{
			 ...
			 */
			// t \in [0;1]
			t /= shackTimestepControl->maxSimulationTime;

			// velocity scalar for effects across entire simulation time (e.g. full revelation around sphere2D)
			// reference solution is computed with T = 5.0 and we resemble it here
			// using radius 1, T=5 would result in u0=1
			double u0 = shackSphere2DDataOps->sphere2d_radius*5.0/shackTimestepControl->maxSimulationTime;

			using namespace sweet::Data::Vector::DataVector_ops;
		...
		)
		{
			...
			double u0_rot = shackSphere2DDataOps->sphere2d_radius/shackTimestepControl->maxSimulationTime;
		...
		if (benchmark_name == "nair_lauritzen_case_4_ext_3")
		{
			...
			/*
			 * Scale things up for the sphere2D
			 */
			...
			// velocity scalar for effects across entire simulation time (e.g. full revelation around sphere2D)
			// reference solution is computed with T = 5.0 and we resemble it here
			double u0 = shackSphere2DDataOps->sphere2d_radius*5.0/T;
			...
			o_u_phys.grid_update_lambda(
				[&](double i_lambda, double i_theta, double &io_data)
				{
					...
					io_data += 2.0*pi*cos(i_theta)*shackSphere2DDataOps->sphere2d_radius/T;
				}
			);
	...
	{
		((PDEAdvectionSphere2DBenchmark_nair_lauritzen_sl*)i_this)->sl_compute_departure_3rd_order(i_pos_lon_A, i_pos_lat_A, o_pos_lon_D, o_pos_lat_D, i_dt, i_timestamp_arrival);
	}
	...
	)
	{
		if (benchmark_name == "nair_lauritzen_case_1")
		{
			...
			double t = i_timestamp_arrival*5.0/shackTimestepControl->maxSimulationTime;

			// rescale time step size
			double dt = i_dt*5.0/shackTimestepControl->maxSimulationTime;
			...
			const sweet::Data::Vector::Vector<double> &lambda = i_pos_lon_A;
			const sweet::Data::Vector::Vector<double> &theta = i_pos_lat_A;

			// eq. (37)
			using namespace sweet::Data::Vector::DataVector_ops;

			sweet::Data::Vector::Vector<double> u_tilde = 2.0*k*pow2(sin(lambda/2))*sin(theta)*cos(pi*t/T);

			// between eq. (34) and (35)
			sweet::Data::Vector::Vector<double> v = k/2.0*sin(lambda)*cos(theta)*cos(omega*t);
		...
		SWEETErrorFatal("TODO: Implement it for this test case");
	}
