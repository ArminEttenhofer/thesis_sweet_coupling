...
//#include <programs/PDE_SWECart2D/time/SWE_Cart2D_TS_l_direct.hpp>
...
bool SWE_Cart2D_TS_l_direct::setup(
...
)
{
	...
	if (shackCart2DDataOps->space_grid_use_c_staggering)
		cart2DDataGridMapping.setup(shackCart2DDataOps, ops->cart2DDataConfig);
...
void SWE_Cart2D_TS_l_direct::runTimestep(
 ...
 */
void SWE_Cart2D_TS_l_direct::run_timestep_cgrid(
		sweet::Data::Cart2D::DataSpectral &io_h_pert,	//!< prognostic variables
		sweet::Data::Cart2D::DataSpectral &io_u,		//!< prognostic variables
		sweet::Data::Cart2D::DataSpectral &io_v,		//!< prognostic variables
...
)
{
	...
	sweet::Data::Cart2D::DataGrid t_u(io_h_pert.cart2DDataConfig);
	sweet::Data::Cart2D::DataGrid t_v(io_h_pert.cart2DDataConfig);

	if (!shackCart2DDataOps->space_grid_use_c_staggering)
		SWEETErrorFatal("Expected staggering");

	cart2DDataGridMapping.mapCtoA_u(io_u.toGrid(), t_u);
	cart2DDataGridMapping.mapCtoA_v(io_v.toGrid(), t_v);

	shackCart2DDataOps->space_grid_use_c_staggering = false;

	sweet::Data::Cart2D::DataSpectral t_u_spec(io_h_pert.cart2DDataConfig);
	sweet::Data::Cart2D::DataSpectral t_v_spec(io_h_pert.cart2DDataConfig);
	t_u_spec.loadCart2DDataGrid(t_u);
	t_v_spec.loadCart2DDataGrid(t_v);
	...
	shackCart2DDataOps->space_grid_use_c_staggering = true;

	cart2DDataGridMapping.mapAtoC_u(t_u_spec.toGrid(), t_u);
	cart2DDataGridMapping.mapAtoC_v(t_v_spec.toGrid(), t_v);

	io_u.loadCart2DDataGrid(t_u);
	io_v.loadCart2DDataGrid(t_v);
}



void SWE_Cart2D_TS_l_direct::run_timestep_agrid(
		sweet::Data::Cart2D::DataSpectral &io_h_pert,	//!< prognostic variables
		sweet::Data::Cart2D::DataSpectral &io_u,	//!< prognostic variables
		sweet::Data::Cart2D::DataSpectral &io_v,	//!< prognostic variables
...
)
{
	...
	run_timestep_agrid_cart2ddata(io_h_pert, io_u, io_v, i_dt, i_simulation_timestamp);
#else
	run_timestep_agrid_cart2ddatacomplex(io_h_pert, io_u, io_v, i_dt, i_simulation_timestamp);
#endif
}
 ...
 */
void SWE_Cart2D_TS_l_direct::run_timestep_agrid_cart2ddata(
		sweet::Data::Cart2D::DataSpectral &io_h_pert,	//!< prognostic variables
		sweet::Data::Cart2D::DataSpectral &io_u,	//!< prognostic variables
		sweet::Data::Cart2D::DataSpectral &io_v,	//!< prognostic variables
...
)
{
	...
	/*
	 * This implementation works directly on sweet::Data::Cart2D::DataSpectral
	 */
	...
	if (compute_phin)
	{
		if (shackExpIntegration->direct_precompute_phin)
		{
			for (std::size_t ik1 = 0; ik1 < io_h_pert.cart2DDataConfig->spectral_data_size[1]; ik1++)
			{
				...
				//}
				for (std::size_t ik0 = 0; ik0 < io_h_pert.cart2DDataConfig->spectral_data_size[0]; ik0++)
				{
					//if (i_simulation_timestamp == 0)
					//{
	...
	for (std::size_t ik1 = 0; ik1 < io_h_pert.cart2DDataConfig->spectral_data_size[1]; ik1++)
	{
		for (std::size_t ik0 = 0; ik0 < io_h_pert.cart2DDataConfig->spectral_data_size[0]; ik0++)
		{
			...
			T k1;
			if (ik1 < io_h_pert.cart2DDataConfig->spectral_data_size[1]/2)
				k1 = (T)ik1;
			else
				k1 = (T)((int)ik1-(int)io_h_pert.cart2DDataConfig->spectral_data_size[1]);
...
void SWE_Cart2D_TS_l_direct::run_timestep_agrid_cart2ddatacomplex(
		sweet::Data::Cart2D::DataSpectral &io_h_pert,	//!< prognostic variables
		sweet::Data::Cart2D::DataSpectral &io_u,	//!< prognostic variables
		sweet::Data::Cart2D::DataSpectral &io_v,	//!< prognostic variables
...
{
	if (i_dt < 0)
		SWEETErrorFatal("SWE_Cart2D_TS_l_direct: Only constant time step size allowed");
	...
	sweet::Data::Cart2DComplex::DataSpectral i_h_pert = sweet::Data::Cart2D::Convert::DataSpectral_2_Cart2DComplex_DataSpectral::grid_convert(io_h_pert);
	sweet::Data::Cart2DComplex::DataSpectral i_u = sweet::Data::Cart2D::Convert::DataSpectral_2_Cart2DComplex_DataSpectral::grid_convert(io_u);
	sweet::Data::Cart2DComplex::DataSpectral i_v = sweet::Data::Cart2D::Convert::DataSpectral_2_Cart2DComplex_DataSpectral::grid_convert(io_v);


	sweet::Data::Cart2DComplex::DataSpectral o_h_pert(io_h_pert.cart2DDataConfig);
	sweet::Data::Cart2DComplex::DataSpectral o_u(io_h_pert.cart2DDataConfig);
	sweet::Data::Cart2DComplex::DataSpectral o_v(io_h_pert.cart2DDataConfig);
	...
	SWEET_THREADING_SPACE_PARALLEL_FOR_SIMD_COLLAPSE2
	for (std::size_t ik1 = 0; ik1 < i_h_pert.cart2DDataConfig->spectral_complex_data_size[1]; ik1++)
	{
		for (std::size_t ik0 = 0; ik0 < i_h_pert.cart2DDataConfig->spectral_complex_data_size[0]; ik0++)
		{
			...
			T k1;
			if (ik1 < i_h_pert.cart2DDataConfig->spectral_complex_data_size[1]/2)
				k1 = (T)ik1;
			else
				k1 = -(T)((int)ik1-(int)i_h_pert.cart2DDataConfig->spectral_complex_data_size[1]);

			T k0;
			if (ik0 < i_h_pert.cart2DDataConfig->spectral_complex_data_size[0]/2)
				k0 = (T)ik0;
			else
				k0 = (T)((int)ik0-(int)i_h_pert.cart2DDataConfig->spectral_complex_data_size[0]);
	...
	io_h_pert = sweet::Data::Cart2DComplex::Convert::DataSpectral_2_Cart2D_DataSpectral::convert_real(o_h_pert);
	io_u = sweet::Data::Cart2DComplex::Convert::DataSpectral_2_Cart2D_DataSpectral::convert_real(o_u);
	io_v = sweet::Data::Cart2DComplex::Convert::DataSpectral_2_Cart2D_DataSpectral::convert_real(o_v);
}
