 ...
 */
#ifndef PROGRAMS_PDE_SWECART2D_BENCHMARKS_PDESWECART2DBENCH_UNSTABLEJET_HPP
#define PROGRAMS_PDE_SWECART2D_BENCHMARKS_PDESWECART2DBENCH_UNSTABLEJET_HPP
		...
		for (int j = 0; j < shackCart2DDataOps->space_res_physical[1]; j++)
		{
			for (int i = 1; i < shackCart2DDataOps->space_res_physical[0]; i++)
			{
				...
				if (i_with_bump)
					...
					depth_phys.grid_setValue(j, i, depth_phys.grid_get(j, 0));
			}
		}
