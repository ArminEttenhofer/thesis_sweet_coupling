...
{
	_shackPDESWECart2D = i_val._shackPDESWECart2D;
	_shackCart2DDataOps = i_val._shackCart2DDataOps;
	_shackExpIntegration = i_val._shackExpIntegration;
	pdeSWECart2DNormalModes = i_val.pdeSWECart2DNormalModes;
	_ops = i_val._ops;
}
...
)
{
...
#if 1
		o_U.u = -_shackPDESWECart2D->gravitation*_ops->diff_c_x(i_U.h_pert) + _shackPDESWECart2D->cart2d_rotating_f0*i_U.v;
		o_U.v = -_shackPDESWECart2D->gravitation*_ops->diff_c_y(i_U.h_pert) - _shackPDESWECart2D->cart2d_rotating_f0*i_U.u;
	...
	else // shackDict.disc.use_staggering = true
	{
		...
		o_u_t_phys = _ops->avg_f_y(_shackPDESWECart2D->cart2d_rotating_f0*_ops->avg_b_x(o_U.v.toGrid())) - _ops->diff_b_x(H).toGrid();
		o_v_t_phys = -_ops->avg_f_x(_shackPDESWECart2D->cart2d_rotating_f0*_ops->avg_b_y(o_U.u.toGrid())) - _ops->diff_b_y(H).toGrid();
	...
	for (std::size_t ik1 = 0; ik1 < _ops->cart2DDataConfig->spectral_data_size[1]; ik1++)
	{
		for (std::size_t ik0 = 0; ik0 < _ops->cart2DDataConfig->spectral_data_size[0]; ik0++)
		{
			...
			if (compute_phin)
			{
				...
				// Eigendecomposition considering the full linear term
				pdeSWECart2DNormalModes.sw_eigen_decomp(k0, k1, false, v, lambda);
				pdeSWECart2DNormalModes.sw_eigen_decomp(k0, k1, true, v_inv, lambda);
