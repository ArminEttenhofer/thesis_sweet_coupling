#ifndef PROGRAMS_LIBPFASST_SHACKLIBPFASST_HPP
#define PROGRAMS_LIBPFASST_SHACKLIBPFASST_HPP
...
private:
	...
	void _set_hyperviscosity_fields(const std::string & hv_fields)
	{
		...
		std::vector<std::string> substrings = sweet::Tools::StringSplit::split(hv_fields, ",");
		...
		for (const auto & substring : substrings)
		{
			...
			{
				SWEETErrorFatal("'" + substring + "' not a known field for hyperviscosity!");
			}
...
public:
	...
	void postprocess_hyperviscosity()
	{
		...
		{
			sweet::Tools::StringSplit::split_n_doubles(hyperviscosity_2_str, hyperviscosity_2, delimiter);
		}
		if (hyperviscosity_4_str != "")
		{
			sweet::Tools::StringSplit::split_n_doubles(hyperviscosity_4_str, hyperviscosity_4, delimiter);
		}
		if (hyperviscosity_6_str != "")
		{
			sweet::Tools::StringSplit::split_n_doubles(hyperviscosity_6_str, hyperviscosity_6, delimiter);
		}
		if (hyperviscosity_8_str != "")
		{
			sweet::Tools::StringSplit::split_n_doubles(hyperviscosity_8_str, hyperviscosity_8, delimiter);
		}
	}
	...
	void postprocess_nsweeps()
	{
		...
		if (nsweeps_str != "")
		{
			sweet::Tools::StringSplit::split_n_ints(nsweeps_str, nsweeps, delimiter);
		}
	}
