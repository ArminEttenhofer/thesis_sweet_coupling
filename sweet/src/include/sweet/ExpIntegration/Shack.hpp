 ...
 */
#ifndef INCLUDE_SWEET_EXPINTEGRATION_SHACK_HPP
#define INCLUDE_SWEET_EXPINTEGRATION_SHACK_HPP
...
public:
	 ...
	 */
	bool sphere2d_solver_preallocation = true;
	 ...
	 * Reduction to half
	 */
	int terry_reduce_2_half = 0;


	/**
	 * Normalization for geostrophic modes
	 */
	int rexi_normalization = 0;
...
public:
	...
	void printProgramArguments(const std::string& i_prefix = "") override
	{
		...
		std::cout << "	--rexi-sphere2d-preallocation [bool]	Use preallocation of SPH-REXI solver coefficients, default:1" << std::endl;
		std::cout << std::endl;
		...
		std::cout << "" << std::endl;
		std::cout << " REXI General:" << std::endl;
		std::cout << "	--rexi-normalization [int]	REXI normalization" << std::endl;
		std::cout << "  EXP direct:" << std::endl;
		std::cout << "	--exp-direct-precompute-phin [int]	Precompute phin in direct exp. solver (only available for SWE on the cart2d), default:0" << std::endl;
		std::cout << std::endl;
	}
	...
	bool processProgramArguments(sweet::Tools::ProgramArguments &i_pa) override
	{
		...
		i_pa.getArgumentValueByKey("--rexi-sphere2d-preallocation", sphere2d_solver_preallocation);
		...
		i_pa.getArgumentValueByKey("--rexi-terry-reduce-to-half", terry_reduce_2_half);
		...
		i_pa.getArgumentValueByKey("--rexi-normalization", rexi_normalization);
		...
		if (rexi_files_given)
		{
			...
			for (std::size_t i = 0; i < split_rexi_files.size(); i++)
			{
				...
				else
				{
					...
					SWEETErrorFatal("Invalid format");
				}
			}
		...
		if (exp_method != "" && exp_method == "terry" && exp_method == "file")
			SWEETErrorFatal("Invalid argument for '--rexi-method='");
	...
	) override
	{
		...
		std::cout << " + rexi_sphere2d_solver_preallocation: " << sphere2d_solver_preallocation << std::endl;
