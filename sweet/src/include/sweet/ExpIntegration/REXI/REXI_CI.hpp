...
#ifndef INCLUDE_SWEET_EXPINTEGRATION_REXI_REXI_CI_HPP
#define INCLUDE_SWEET_EXPINTEGRATION_REXI_REXI_CI_HPP
	...
	)
	{
		...
		if (i_primitive_name == "circle")
		{
			if (i_size_real != i_size_imag)
				SWEETErrorFatal("size along real and imaginary axis must be the same");
			...
			for (int j = 0; j < N; j++)
			{
				...
				TComplex pos = r*LibMath::DQStuff::exp(I*theta_j);
				...
				std::complex<T> tmp;
				fun.eval(gamma_j, tmp);
				beta_eval[j] = filter_value*tmp*pos;
		...
		{
//			SWEETErrorFatal("Not yet working :-(");
			...
			if (total_N > N)
				SWEETErrorFatal("Total number of points exceeds the total number of requested points");
		...
		{
			SWEETErrorFatal("This primitive is not known");
		}
