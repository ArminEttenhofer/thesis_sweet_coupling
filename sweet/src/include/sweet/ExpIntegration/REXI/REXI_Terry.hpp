 ...
 */
#ifndef INCLUDE_SWEET_EXPINTEGRATION_REXI_REXI_TERRY_HPP
#define INCLUDE_SWEET_EXPINTEGRATION_REXI_REXI_TERRY_HPP
...
#include <algorithm>
#include <assert.h>
#include <sweet/ExpIntegration/REXI/trexi/REXI_Terry_ExponentialApproximation.hpp>
#include <sweet/ExpIntegration/REXI/trexi/REXI_Terry_FunApproximation.hpp>
#include <sweet/ExpIntegration/REXI/trexi/REXI_Terry_GaussianApproximation.hpp>
...
public:
	REXI_Terry(
			...
			bool i_reduce_2_half = true
	)
	{
		setup(i_function_id, i_h, i_M, i_L, i_reduce_2_half);
	}
...
public:
	void setup(
		...
		bool i_reduce_2_half = true	//!< reduce the number of poles to half
	)
	{
		trexi::REXI_Terry_GaussianApproximation<T> ga(i_L);
		...
		if (i_h < 0)
			SWEETErrorFatal("Specify the sampling distance width of REXI (parameter --rexi-h)");

		if (i_function_name == "phi0")
		{
			trexi::REXI_Terry_ExponentialApproximation<T> exp_approx(i_h, i_M);
			b = exp_approx.b;
		}
		else
		{
			trexi::REXI_Terry_FunApproximation<T> phia(i_function_name, i_h, i_M);
			b = phia.b;
		}
...
//		rexiFunctions.setup(i_function_name);
		...
		for (int n = -N; n < N+1; n++)
		{
			...
			{
				SWEET_ASSERT(k+L >= 0);
				SWEET_ASSERT(k+L < 2*L+1);
				SWEET_ASSERT(n-k+M >= 0);
				SWEET_ASSERT(n-k+M < 2*M+1);
	...
	)
	{
		...
		if (!i_linear_operator)
		{
			for (std::size_t n = 0; n < S; n++)
				sum += (LibMath::DQStuff::convertComplex<TStorage>(beta[n]) / (TStorageComplex(0, i_x) + sweet::LibMath::DQStuff::convertComplex<TStorage>(alpha[n])));
		}
		else
		{
			...
			for (std::size_t n = 0; n < S; n++)
			{
				...
				L[0][0] += sweet::LibMath::DQStuff::convertComplex<TStorage>(alpha[n]);
				L[1][1] += sweet::LibMath::DQStuff::convertComplex<TStorage>(alpha[n]);
				...
				sum += sweet::LibMath::DQStuff::Re(ret[0]) + sweet::LibMath::DQStuff::Re(ret[1])*LibMath::DQStuff::I((TStorage)1.0);
			}
		}
	...
	)
	{
		...
		for (std::size_t n = 0; n < S; n++)
			sum += (LibMath::DQStuff::convertComplex<T>(beta[n]) / (TComplex(0, i_x) + sweet::LibMath::DQStuff::convertComplex<T>(alpha[n]))).real();
	...
	)
	{
		...
		std::complex<T> sum = 0;
		for (std::size_t n = 0; n < S; n++)
			sum += (LibMath::DQStuff::convertComplex<T>(beta[n]) / (TComplex(0, i_x) + sweet::LibMath::DQStuff::convertComplex<T>(alpha[n])));
		return sum;
	}
};
