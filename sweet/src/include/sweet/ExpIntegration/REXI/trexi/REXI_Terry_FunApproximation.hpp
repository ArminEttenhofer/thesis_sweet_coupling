 ...
 */
#ifndef INCLUDE_SWEET_EXPINTEGRATION_REXI_TREXI_REXI_TERRY_FUNAPPROXIMATION_HPP
#define INCLUDE_SWEET_EXPINTEGRATION_REXI_TREXI_REXI_TERRY_FUNAPPROXIMATION_HPP
...
public:
	...
	T heaviside(T i_x)
	{
		...
		SWEETErrorFatal("Heaviside not defined at 0");
		return 0;
	}
	...
	)
	{
		...
		{
			std::complex<T> term = -pi*h*sweet::LibMath::DQStuff::exp(
	...
	)
	{
		 ...
		 */
		T psi_h = h*sweet::LibMath::DQStuff::exp(-h*h*i_xi*i_xi*pi2*pi2);
		return h * sweet::LibMath::DQStuff::expIm(pi2*(T)m*h*i_xi)/psi_h * fun_phi_tilde(i_xi*pi2);
	...
	{
		T real = sweet::LibMath::GaussQuadrature::integrate5_intervals_adaptive_recursive<T>(
		...
		T imag = sweet::LibMath::GaussQuadrature::integrate5_intervals_adaptive_recursive<T>(
...
//				b[m+M] = computeREXICoefficient(m, fun, -1.0/pi2, -1e-12);
		...
		{
			SWEETErrorFatal(std::string("Function ")+i_function_name+std::string(" is not supported"));
		}
	}
