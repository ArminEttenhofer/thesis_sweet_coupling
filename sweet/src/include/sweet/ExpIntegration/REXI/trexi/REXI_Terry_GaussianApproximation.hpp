 ...
 */
#ifndef INCLUDE_SWEET_EXPINTEGRATION_REXI_TREXI_REXI_TERRY_GAUSSIANAPPROXIMATION_HPP
#define INCLUDE_SWEET_EXPINTEGRATION_REXI_TREXI_REXI_TERRY_GAUSSIANAPPROXIMATION_HPP
 ...
 */
template <typename T>
class REXI_Terry_GaussianApproximation
{
...
public:
	T mu;					//!< average
	std::vector<TComplex> a;	//!< weights for approximation
	int L;							//!< 2*L+1 = number of weights

	REXI_Terry_GaussianApproximation(
			int i_L = 0	//!< L
	)
	{
		...
		{
			// T == __float128
			pi = sweet::LibMath::DQStuff::fromString<T>("3.14159265358979323846264338327950288");
			pi2 = pi*LibMath::DQStuff::fromString<T>("2.0");
			pi4 = pi*LibMath::DQStuff::fromString<T>("4.0");
			sqrtpi4 = sweet::LibMath::DQStuff::sqrt(pi4);
		}
		else
#endif
		if (sizeof(T) == 8)
		...
		{
			SWEETErrorFatal("Type T not supported");
		}
...
#if 0
			mu = sweet::LibMath::DQStuff::fromString<T>("-4.315321510875024");

			a[0].real(LibMath::DQStuff::fromString<T>("-1.0845749544592896e-7"));		a[0].imag(LibMath::DQStuff::fromString<T>("2.77075431662228e-8"));
			a[1].real(LibMath::DQStuff::fromString<T>("1.858753344202957e-8"));		a[1].imag(LibMath::DQStuff::fromString<T>("-9.105375434750162e-7"));
			a[2].real(LibMath::DQStuff::fromString<T>("3.6743713227243024e-6"));		a[2].imag(LibMath::DQStuff::fromString<T>("7.073284346322969e-7"));
			a[3].real(LibMath::DQStuff::fromString<T>("-2.7990058083347696e-6"));		a[3].imag(LibMath::DQStuff::fromString<T>("0.0000112564827639346"));
			a[4].real(LibMath::DQStuff::fromString<T>("0.000014918577548849352"));	a[4].imag(LibMath::DQStuff::fromString<T>("-0.0000316278486761932"));
			a[5].real(LibMath::DQStuff::fromString<T>("-0.0010751767283285608"));		a[5].imag(LibMath::DQStuff::fromString<T>("-0.00047282220513073084"));
			a[6].real(LibMath::DQStuff::fromString<T>("0.003816465653840016"));		a[6].imag(LibMath::DQStuff::fromString<T>("0.017839810396560574"));
			a[7].real(LibMath::DQStuff::fromString<T>("0.12124105653274578"));		a[7].imag(LibMath::DQStuff::fromString<T>("-0.12327042473830248"));
			a[8].real(LibMath::DQStuff::fromString<T>("-0.9774980792734348"));		a[8].imag(LibMath::DQStuff::fromString<T>("-0.1877130220537587"));
			a[9].real(LibMath::DQStuff::fromString<T>("1.3432866123333178"));			a[9].imag(LibMath::DQStuff::fromString<T>("3.2034715228495942"));
			a[10].real(LibMath::DQStuff::fromString<T>("4.072408546157305"));			a[10].imag(LibMath::DQStuff::fromString<T>("-6.123755543580666"));
			a[11].real(LibMath::DQStuff::fromString<T>("-9.442699917778205"));		a[11].imag(LibMath::DQStuff::fromString<T>("0.0"));
			a[12].real(LibMath::DQStuff::fromString<T>("4.072408620272648"));			a[12].imag(LibMath::DQStuff::fromString<T>("6.123755841848161"));
			a[13].real(LibMath::DQStuff::fromString<T>("1.3432860877712938"));		a[13].imag(LibMath::DQStuff::fromString<T>("-3.2034712658530275"));
			a[14].real(LibMath::DQStuff::fromString<T>("-0.9774985292598916"));		a[14].imag(LibMath::DQStuff::fromString<T>("0.18771238018072134"));
			a[15].real(LibMath::DQStuff::fromString<T>("0.1212417070363373"));		a[15].imag(LibMath::DQStuff::fromString<T>("0.12326987628935386"));
			a[16].real(LibMath::DQStuff::fromString<T>("0.0038169724770333343"));		a[16].imag(LibMath::DQStuff::fromString<T>("-0.017839242222443888"));
			a[17].real(LibMath::DQStuff::fromString<T>("-0.0010756025812659208"));	a[17].imag(LibMath::DQStuff::fromString<T>("0.0004731874917343858"));
			a[18].real(LibMath::DQStuff::fromString<T>("0.000014713754789095218"));	a[18].imag(LibMath::DQStuff::fromString<T>("0.000031358475831136815"));
			a[19].real(LibMath::DQStuff::fromString<T>("-2.659323898804944e-6"));		a[19].imag(LibMath::DQStuff::fromString<T>("-0.000011341571201752273"));
			a[20].real(LibMath::DQStuff::fromString<T>("3.6970377676364553e-6"));		a[20].imag(LibMath::DQStuff::fromString<T>("-6.517457477594937e-7"));
			a[21].real(LibMath::DQStuff::fromString<T>("3.883933649142257e-9"));		a[21].imag(LibMath::DQStuff::fromString<T>("9.128496023863376e-7"));
			a[22].real(LibMath::DQStuff::fromString<T>("-1.0816457995911385e-7"));	a[22].imag(LibMath::DQStuff::fromString<T>("-2.954309729192276e-8"));
	...
	{
		return sweet::LibMath::DQStuff::exp(-(x*x)/(4.0*h*h))/sqrtpi4;
	}
	...
	)
	{
		...
		for (int l = 0; l < 2*L+1; l++)
		{
			...
			std::complex<T> aEval(a[l].real(), a[l].imag());

			// WORKS with max error 7.15344e-13
			sum += (aEval/(TComplex(0, x) + (T)mu + TComplex(0, j))).real();
		}
	 ...
	 * with sum of complex rational functions
	 */
	std::complex<T> approxGaussian_Complex(
			T x,	//!< x-coefficient for Gaussian basis function
			T h	//!< h-coefficient for Gaussian basis function
	)
	{
		...
		for (int l = 0; l < 2*L+1; l++)
		{
			...
			sum += (a[l]/(TComplex(0, x) + (T)mu + TComplex(0, j)));
		}
