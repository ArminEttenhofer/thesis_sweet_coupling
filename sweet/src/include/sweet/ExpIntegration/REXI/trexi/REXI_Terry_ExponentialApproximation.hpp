 ...
 */
#ifndef INCLUDE_SWEET_EXPINTEGRATION_REXI_TREXI_REXI_TERRY_EXPONENTIALAPPROXIMATION_HPP
#define INCLUDE_SWEET_EXPINTEGRATION_REXI_TREXI_REXI_TERRY_EXPONENTIALAPPROXIMATION_HPP
...
public:
	...
	)
	{
		...
		for (int m = -i_M; m < i_M+1; m++)
		{
			...
			b[m+M] = sweet::LibMath::DQStuff::exp(
					TComplex(h*h, -(T)m*h)
				);
		}
	}
	...
	{
		return sweet::LibMath::DQStuff::expIm(i_x);
	}
	...
	)
	{
		...
		for (int m = -M; m < M+1; m++)
		{
			sum += sweet::LibMath::DQStuff::convertComplex<T>(b[m+M]) * ga.approxGaussian(i_x+(T)m*h, h);
		}
		return sum;
	}
};
