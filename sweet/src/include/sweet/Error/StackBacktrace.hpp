...
#ifndef INCLUDE_SWEET_ERROR_STACKBACKTRACE_HPP
#define INCLUDE_SWEET_ERROR_STACKBACKTRACE_HPP
...
public:
	/*
	 * We avoid using error::Base to avoid circular dependency
	 */
	std::string error;

private:
	static
	void BacktraceErrorFatal(const std::string &i_msg)
	{
		std::cerr << "Internal error with StackBacktrace:" << std::endl;
...
public:
	...
	std::string getGDBBacktrace()
	{
		...
		if (len == -1)
			BacktraceErrorFatal("Failed to read link");
		...
		if (result_pipe_cout == -1)
			BacktraceErrorFatal("Failed to create cout pipe");

		int pipefd_cerr[2];
		int result_pipe_cerr = pipe(pipefd_cerr);
		if (result_pipe_cerr == -1)
			BacktraceErrorFatal("Failed to create cerr pipe");
		...
		int child_pid = fork();
		if (child_pid == -1)
			BacktraceErrorFatal("Failed to create fork");
