/*
 * Cart2DData_Kernels.hpp
 *
 *  Created on: 19 Oct 2016
 * Author: Martin SCHREIBER <schreiberx@gmail.com>
 */

#ifndef INCLUDE_SWEET_DATA_CART2D_DATAGRID_KERNELS_HPP
#define INCLUDE_SWEET_DATA_CART2D_DATAGRID_KERNELS_HPP
...
/*!
 * \brief Helper class to apply kernels (stencils) on Cart2D grids
 */
class DataGrid_Kernels
{
	int kernel_size = -1;
...
//#if !SWEET_USE_CART2D_SPECTRAL_SPACE
		...
		kernel_size = S;
		kernel_data = sweet::Memory::MemBlockAlloc::alloc<double>(sizeof(double)*S*S);
		for (int y = 0; y < S; y++)
			for (int x = 0; x < S; x++)
		...
		if (S == 3)
		{
			kernel_id = DataGrid_Kernels::get_kernel_mask3x3(
					kernel_data[0] != 0,
					kernel_data[1] != 0,
					kernel_data[2] != 0,
					kernel_data[3] != 0,
...
#define KERNEL_grid_set(j, i, value)		\
	o_grid_data[(j)*cart2DDataConfig->grid_data_size[0]+(i)] = (value);
		...
		for (std::size_t ky = R; ky < S; ky++)
			for (std::size_t kx = R; kx < S; kx++)
			{
				...
				if (0 > cx || (int)cart2DDataConfig->grid_data_size[0] <= cx)
					continue;
				if (0 > cy || (int)cart2DDataConfig->grid_data_size[1] <= cy)
					continue;

				KERNEL_grid_set(cy, cx, inv_kernel_array[ky][kx]);
			}
		...
		for (std::size_t ky = R; ky < S; ky++)
			for (std::size_t kx = 0; kx < R; kx++)
			{
				// coordinates in Cartesian space
				int cy = ky-R;
				int cx = cart2DDataConfig->grid_data_size[0] - R + kx;
		...
		for (std::size_t ky = 0; ky < R; ky++)
			...
			{
				// coordinates in Cartesian space
				int cy = cart2DDataConfig->grid_data_size[1] - R + ky;
				int cx = kx-R;
...
public:
			...
			case get_kernel_mask3x3(0, 0, 0, 1, 0, 1, 0, 0, 0):	// (X, 0, X)
				...
				for (int y = 0; y < res_y; y++)
				{
					for (int x = 0; x < res_x; x++)
					{
						...
						int pos_y = y;
						SWEET_ASSERT(pos_y >= 0 && pos_y < res_y);
					...
					for (int x = 0; x < res_x; x++)
					{
						...
						for (int j = -1; j <= 1; j++)
						{
							...
							for (int i = -1; i <= 1; i++)
							{
								...
								SWEET_ASSERT(pos_x >= 0 && pos_x < res_x);

								int idx = (j+1)*3+(i+1);
								SWEET_ASSERT(idx >= 0 && idx < 9);
	...
	~DataGrid_Kernels()
	{
		if (kernel_data != nullptr)
			sweet::Memory::MemBlockAlloc::free(kernel_data, sizeof(double)*kernel_size*kernel_size);
	}
};
