...
#ifndef INCLUDE_SWEET_TOOLS_STRINGSPLIT_HPP
#define INCLUDE_SWEET_TOOLS_STRINGSPLIT_HPP
...
public:
	...
	)
	{
		...
		{
			SWEETErrorFatal("This number of values is unexpected! String '" + i_str + "' invalid.");
		}
	...
	{
		std::vector<std::string> res = sweet::Tools::StringSplit::split(i_str, ",");

		int c = res.size();

		if (c == 0)
			SWEETErrorFatal("Invalid format for modes");
		...
		SWEETErrorFatal("More than 3 values given");
		return -1;
	}
	...
	)
	{
		...
		SWEETErrorFatal("More than 2 values given");
		return -1;
	}
