#ifndef INCLUDE_SWEET_TIMETREE_INTERIORNODES_ADDINTEGRATION_HPP
#define INCLUDE_SWEET_TIMETREE_INTERIORNODES_ADDINTEGRATION_HPP
...
/*!
 * Add integrations of different tree nodes
 */
class AddIntegration	:
	public TimeTree_Node_InteriorHelper<AddIntegration>
{
public:
	AddIntegration()
	{
	...
	AddIntegration(
			const AddIntegration &i_src
	)	:
		TimeTree_Node_InteriorHelper<AddIntegration>(i_src)
	{
	}
	...
	getNodeNames()	override
	{
		...
		retval.push_back("addIntegration");
		retval.push_back("addI");
		retval.push_back("ADDINTEGRATION");
		retval.push_back("ADDI");
		return retval;
	}
	...
	{
		o_ostream << i_prefix << "InteriorNode: 'AddIntegration':" << std::endl;
		o_ostream << i_prefix << std::endl;
		o_ostream << i_prefix << "  - Node name & aliases: " << _getNodeNamesAsString() << std::endl;
		o_ostream << i_prefix << std::endl;
		o_ostream << i_prefix << "  - Usage: ADDI(d1,d2,d3,...)" << std::endl;
		o_ostream << i_prefix << "           Adds together evaluation of time integration" << std::endl;
	...
	{
		return std::shared_ptr<TimeTree_Node_Base>(new AddIntegration(*this));
	}
