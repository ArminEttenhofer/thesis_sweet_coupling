#ifndef INCLUDE_SWEET_TIMETREE_INTERIORNODES_NEGTENDENCIES_HPP
#define INCLUDE_SWEET_TIMETREE_INTERIORNODES_NEGTENDENCIES_HPP
 ...
 */
class NegTendencies	:
	public TimeTree_Node_InteriorHelper<NegTendencies>
{
public:
	NegTendencies()
	{
	...
	NegTendencies(
			const NegTendencies &i_src
	)	:
		TimeTree_Node_InteriorHelper<NegTendencies>(i_src)
	{
	}
	...
	getNodeNames()	override
	{
		...
		retval.push_back("negT");
		retval.push_back("negTendencies");
		retval.push_back("NEGTENDENCIES");
		retval.push_back("NEGT");
		return retval;
	}
	...
	{
		o_ostream << i_prefix << "InteriorNode: 'NegTendencies':" << std::endl;
		...
		o_ostream << i_prefix << "           Adds together negative evaluation of time tendencies" << std::endl;
	...
	{
		return std::shared_ptr<TimeTree_Node_Base>(new NegTendencies(*this));
	}
