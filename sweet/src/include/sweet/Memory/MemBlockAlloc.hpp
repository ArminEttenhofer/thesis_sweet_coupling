 ...
 *
 */
#ifndef INCLUDE_SWEET_MEMORY_MEMBLOCKALLOC_HPP
#define INCLUDE_SWEET_MEMORY_MEMBLOCKALLOC_HPP
...
private:
	...
	void parse_argv()
	{
		 ...
		 *   alloc=perthread
		 */
		std::vector<std::string> params = sweet::Tools::StringSplit::split(envstring, ",");
		...
		for (auto iter = params.begin(); iter != params.end(); iter++)
		{
			...
			std::vector<std::string> split_params = sweet::Tools::StringSplit::split(param, "=");
...
#if SWEET_DEBUG
				if (omp_get_level() > 1)
					SWEETErrorFatal("Nested parallelism not supported with per-thread block allocator, please use different one!");
#endif
...
public:
	static
	inline
	sweet::Memory::MemBlockAlloc& getSingletonRef()
	{
		/*
		 * Compilation details:
		 *
	...
	{
		sweet::Memory::MemBlockAlloc &n = sweet::Memory::MemBlockAlloc::getSingletonRef();

		SWEET_ASSERT(n.getThreadLocalDomainIdRef() < (int)n._domain_block_groups.size());
...
private:
	void _shutdown()
	{
		...
		for (auto& n : _domain_block_groups)
		{
			...
			{
				//std::cout << "MemBlockAlloc: Cleaning up " << g.free_blocks.size() << " blocks of size " << g.block_size << std::endl;
