...
#ifndef INCLUDE_SWEET_LIBGL_TOOLS_GLCUBEMAP_HPP
#define INCLUDE_SWEET_LIBGL_TOOLS_GLCUBEMAP_HPP
...
{
	virtual void createCallback(mat4 &projection_matrix, mat4 &view_matrix) = 0;
...
#undef prepare
	 ...
	 * create cube map
	 */
	void create(	void (*callback)(mat4 &projection_matrix, mat4 &view_matrix, void *user_data),	//!< pointer to class with callback handlers
					const vec3 &translate,	//!< translation vector of model_matrix (last column)
					...
					)
	{
		...
		mat4 projection_matrix = sweet::libmath::frustum(-near_plane, near_plane, -near_plane, near_plane, near_plane, far_plane);
		mat4 view_matrix = sweet::libmath::rotate(180.0f, 0.0f, 0.0f, 1.0f);
		mat4 p_view_matrix;
		...
		// FRONT
		fbo_neg_z.bind();
		p_view_matrix = view_matrix;//*GLSLMATH::rotate(180.0f, 0.0f, 0.0f, 1.0f);
		p_view_matrix *= sweet::libmath::translate(translate);
		...
		// BACK
		fbo_pos_z.bind();
		p_view_matrix = view_matrix*GLSLMATH::rotate(180.0f, 0.0f, 1.0f, 0.0f);
		p_view_matrix *= sweet::libmath::translate(translate);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		callback(projection_matrix, p_view_matrix, user_data);
		...
		// LEFT
		fbo_neg_x.bind();
		p_view_matrix = view_matrix*GLSLMATH::rotate(-90.0f, 0.0f, 1.0f, 0.0f);
		p_view_matrix *= sweet::libmath::translate(translate);
		...
		fbo_pos_y.bind();
		p_view_matrix = view_matrix*GLSLMATH::rotate(180.0f, 0.0f, 0.0f, 1.0f);
		p_view_matrix *= sweet::libmath::rotate(-90.0f, 1.0f, 0.0f, 0.0f);
		p_view_matrix *= sweet::libmath::translate(translate);
