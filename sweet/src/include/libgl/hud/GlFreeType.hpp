...
#ifndef INCLUDE_SWEET_LIBGL_HUD_GLFREETYPE_HPP
#define INCLUDE_SWEET_LIBGL_HUD_GLFREETYPE_HPP
...
#include "../core/GlVertexArrayObject.hpp"
...
public:
	...
	)
	{
		 ...
		 */
		program.use();
		glyph_size_uniform.set(ivec2(char_width, char_height));
		program.disable();
...
public:
	...
	{
		program.use();
		inv_viewport_size_uniform.set(vec2(1.0/(float)i_width, 1.0/(float)i_height));
		program.disable();
	}
