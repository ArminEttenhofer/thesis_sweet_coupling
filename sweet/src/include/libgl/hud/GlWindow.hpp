...
#ifndef INCLUDE_SWEET_LIBGL_HUD_GLWINDOW_HPP
#define INCLUDE_SWEET_LIBGL_HUD_GLWINDOW_HPP
...
//	CError error;
	...
	ivec2 pos;		//!< position of window
	ivec2 size;		//!< size of window
	...
	GlWindow()
	{
		...
		setBackgroundColor(vec4(74.0/256.0, 96.0/256.0, 150.0/256.0, 0.7));
	}

	/**
	 * set the window background color
	 */
	void setBackgroundColor(const vec4 &p_background_color)
	{
		program.use();
	...
	void setSize(const ivec2 &p_size)
	{
		size = p_size;
	}
	void setPosition(const ivec2 &p_pos)
	{
		pos = p_pos;
	}
	...
	void startRendering()
	{
...

		{
			...
			program.use();
			draw_quad.renderWithoutProgram();
			program.disable();
		}
	}
