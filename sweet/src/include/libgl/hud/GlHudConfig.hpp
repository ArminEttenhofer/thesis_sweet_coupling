...
#ifndef INCLUDE_SWEET_LIBGL_HUD_GLHUDCONFIG_HPP
#define INCLUDE_SWEET_LIBGL_HUD_GLHUDCONFIG_HPP
...
public:
	...
	void renderConfigContent()
	{
		...
		cGlFreeType->setColor(vec3(1,1,1));
		CGlErrorCheck();
		...
		for (std::list<COption>::iterator i = option_list.begin(); i != option_list.end(); i++)
		{
			...
			if (o.button_left_down)
				cGlFreeType->setColor(vec3(0.7,1,0.7));
			else if (o.in_area)
				cGlFreeType->setColor(vec3(1,0.5,0.5));
