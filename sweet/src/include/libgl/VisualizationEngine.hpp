 ...
 */
#ifndef INCLUDE_SWEET_LIBGL_VISUALIZATIONENGINE_HPP
#define INCLUDE_SWEET_LIBGL_VISUALIZATIONENGINE_HPP
...
public:
	...
	public:
		...
		virtual bool vis_shouldQuit() = 0;

		virtual const std::string vis_getStatusString(bool &o_replace_commas_with_newline) = 0;
...
//					engineState->modelEyeBall.rotate(speed*(-old_mouse_x + i_x), vec3(0, 1, 0));
...
public:
			...
			programCallbacks(&(ProgramCallbacks&)*i_simulationClassWithProgramCallbacks)
	{
		...
		while (!engineState->quit)
		{
			...
			engineState->camera1stPerson.setFrustum(-fzoom*renderWindow->aspect_ratio, fzoom*renderWindow->aspect_ratio, -fzoom, fzoom, engineState->near_plane, engineState->far_plane);
			 ...
			 * LIGHTING
			 */
			vec4 v = engineState->camera1stPerson.view_matrix
					*vec3(
			...
			engineState->commonShaderPrograms.shaderBlinn.material_ambient_color3_uniform.set(vec3(0.1,0.1,0.2));
			engineState->commonShaderPrograms.shaderBlinn.material_diffuse_color3_uniform.set(vec3(0.1,0.1,1.0));
			engineState->commonShaderPrograms.shaderBlinn.material_specular_color3_uniform.set(vec3(1.0,1.0,1.0));
			...
			engineState->quit |= programCallbacks->vis_shouldQuit();

			bool o_replace_commas_with_newline;
			renderWindow->setWindowTitle(programCallbacks->vis_getStatusString(o_replace_commas_with_newline));
