...
static char *cpuset_2_cstr(cpu_set_t *mask, char *str)
{
	char *ptr = str;
...
void schedInfo(
		...
		std::map<int, int> &tid_2_worker_id,
...
)
{
	...
	int worker_id = tid_2_worker_id[tid];

	cpu_set_t coremask;
	sched_getaffinity(0, sizeof(coremask), &coremask);

	char cores_buf[1024];
	cpuset_2_cstr(&coremask, cores_buf);
...
#pragma omp critical
	...
	if (true)
	{
		...
		#pragma omp parallel
		{
			...
			ss_iter << "(i=" << i << ")";
			schedInfo(ss, tid_2_worker_id, ss_iter.str());
