...
bool preciceAdapter::Adapter::configFileRead() {
    ...
    try {
        ...
        vertical_samples = preciceDict.get<int>("vertical_samples");
        ...
        if (!interfaceDictPtr) {
            ...
            for (const entry &interfaceDictEntry: *interfaceDictPtr) {
                DEBUG(adapterInfo("New Interface Entry"));
                if (interfaceDictEntry.isDict()) {
                    ...
                    if (interfaceConfig.meshConnectivity && (interfaceConfig.locationsType == "faceCenters" ||
                                                             interfaceConfig.locationsType == "volumeCenters" ||
                                                             interfaceConfig.locationsType == "volumeCentres")) {
                    ...
                    if (!interfaceConfig.cellSetNames.empty() && !(interfaceConfig.locationsType == "volumeCenters" ||
                                                                   interfaceConfig.locationsType == "volumeCentres")) {
        ...
        DEBUG(adapterInfo("Finish config file read"));
...
void preciceAdapter::Adapter::configure() {
    ...
    try {
        ...
        precice_ = new precice::Participant(participantName_, preciceConfigFilename_, Pstream::myProcNo(),
                                            Pstream::nProcs());
        ...
        for (uint i = 0; i < interfacesConfig_.size(); i++) {
            ...
            Interface *interface = new Interface(*precice_, mesh_, interfacesConfig_.at(i).meshName,
                                                 interfacesConfig_.at(i).locationsType,
                                                 interfacesConfig_.at(i).patchNames,
                                                 interfacesConfig_.at(i).cellSetNames,
                                                 interfacesConfig_.at(i).meshConnectivity, restartFromDeformed,
                                                 namePointDisplacement, nameCellDisplacement);
            ...
            for (auto dataName: interfacesConfig_.at(i).writeData) {
                if (dataName.find("Alpha") == 0) alphaName = dataName;
                if (dataName.find("Velocity") == 0) velName = dataName;
            }

            if (alphaName != "" && velName != "") {
                try {
                    dimensionReduction.emplace_back(mesh_, "alpha.water", "U", interface->patchIDs_, vertical_samples);
            ...
            for (uint j = 0; j < interfacesConfig_.at(i).writeData.size(); j++) {
                ...
                adapterInfo("Handle data " + dataName);
                ...
                if (FFenabled_ && FF_->addWriters(dataName, interface, dimensionReduction[0])) {
                    inModules++;
                }
            ...
            for (uint j = 0; j < interfacesConfig_.at(i).readData.size(); j++) {
                ...
                if (FFenabled_ && FF_->addReaders(dataName, interface, dimensionReduction[0])) inModules++;

                adapterInfo("Added data readers");
        ...
        adapterInfo("Finished creating interfaces");
    ...
    if (precice_->getMaxTimeStepSize() - timestepSolverDetermined > tolerance) {
        ...
        if (precice_->isCouplingOngoing()) {
            adapterInfo(
                    ...
                    " Adjusting from "
                    + std::to_string(timestepSolverDetermined) + " to " +
                    std::to_string(precice_->getMaxTimeStepSize()),
                    "warning");
        }
...
void preciceAdapter::Adapter::readMeshCheckpoint() {
    ...
    for (uint i = 0; i < meshSurfaceScalarFields_.size(); i++) {
        ...
        *(meshSurfaceScalarFields_.at(i)) == *(meshSurfaceScalarFieldCopies_.at(i));
        ...
        if (nOldTimes == 2) {
            meshSurfaceScalarFields_.at(i)->oldTime().oldTime() ==
            meshSurfaceScalarFieldCopies_.at(i)->oldTime().oldTime();
        }
    }
    ...
    for (uint i = 0; i < meshSurfaceVectorFields_.size(); i++) {
        ...
        *(meshSurfaceVectorFields_.at(i)) == *(meshSurfaceVectorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2) {
            meshSurfaceVectorFields_.at(i)->oldTime().oldTime() ==
            meshSurfaceVectorFieldCopies_.at(i)->oldTime().oldTime();
        }
    }
...
preciceAdapter::Adapter::~Adapter() {
    ...
    // Continuing the output started in the destructor of preciceAdapterFunctionObject
            Info << "Time exclusively in the adapter: "
                 << (timeInConfigRead_ + timeInMeshSetup_ + timeInCheckpointingSetup_ + timeInWrite_ + timeInRead_ +
                     timeInCheckpointingWrite_ + timeInCheckpointingRead_).str() << nl;
            ...
            Info << "  (I) writing OpenFOAM results:  " << timeInWriteResults_.str()
                 << " (at the end of converged time windows)" << nl << nl;
            Info << "Time exclusively in preCICE:     " << (timeInInitialize_ + timeInAdvance_ + timeInFinalize_).str()
                 << nl;
            ...
            Info << "  See also precice-profiling on the website https://precice.org/tooling-performance-analysis.html."
                 << nl;
            Info << "-------------------------------------------------------------------------------------" << nl;)
