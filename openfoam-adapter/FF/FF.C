...
bool preciceAdapter::FF::FluidFluid::addWriters(std::string dataName, Interface* interface, DimensionReduction& dimRed)
{
    adapterInfo("Add writer");
    ...
    else if (dataName.find("Velocity") == 0)
    {
        interface->addCouplingDataWriter(
            dataName,
            new Velocity(mesh_, nameU_, namePhi_, dimRed, fluxCorrection_));
    ...
    else if (dataName.find("Alpha") == 0)
    {
        interface->addCouplingDataWriter(
            dataName,
            new Alpha(mesh_, nameAlpha_, dimRed));
...
bool preciceAdapter::FF::FluidFluid::addReaders(std::string dataName, Interface* interface, DimensionReduction& dimRed)
{
    bool found = true; // Set to false later, if needed.
    ...
    else if (dataName.find("Velocity") == 0)
    {
        interface->addCouplingDataReader(
            dataName,
            new Velocity(mesh_, nameU_, namePhi_, dimRed));
